...
ARTICLE_MAX_CHARS = int(os.getenv("SOVNODE_ARTICLE_MAX_CHARS", "1000"))
MAX_ARTICLES_TO_SCRAPE = int(os.getenv("SOVNODE_MAX_SCRAPE", "2"))
...
WIKI_FINAL_EXTRACT_CHARS = int(os.getenv("SOVNODE_WIKI_FINAL_EXTRACT_CHARS", "2500"))
...
MAX_TOTAL_CONTEXT_CHARS = int(os.getenv("SOVNODE_MAX_CONTEXT_CHARS", "4000"))
HARD_TIMEOUT_FALLBACK_SECONDS: float = float(os.getenv("SOVNODE_SEARCH_HARD_FALLBACK", "600.0"))

IMAGE_SEARCH_TIMEOUT_SECONDS: float = float(os.getenv("SOVNODE_IMAGE_SEARCH_TIMEOUT", "3.5"))
IMAGE_SEARCH_MAX_RESULTS = int(os.getenv("SOVNODE_IMAGE_SEARCH_MAX_RESULTS", "3"))
...
def _fetch_article_content_and_image(url: str, query: str = "") -> Dict[str, Optional[str]]:
    ...
    try:
        ...
        if downloaded:
            ...
            # BLINDAJE (bug real, MEDIDO 2026-09-07 — auditoría de
            # búsqueda web): esta función promete traer texto Y foto
            # ("_and_image" en el nombre), pero nunca scrapeaba ninguna
            # imagen — `result["image"]` quedaba en `None` SIEMPRE, para
            # CUALQUIER artículo, sin importar el tema. El llamador
            # (`search_web`, más abajo) solo rellenaba con el favicon
            # diminuto de `_favicon_url()` cuando `item.get("image")`
            # venía vacío — que era 100% de las veces — así que ninguna
            # fuente de la Capa 1 llegaba nunca con una foto real,
            # independientemente de si el tema pasaba el filtro de la
            # tarjeta visual o no (visto en vivo: hasta una consulta de
            # terremoto, que sí pasa ese filtro, terminaba en "ninguna
            # fuente trajo una miniatura real"). `extract_metadata()` de
            # trafilatura sí expone el og:image/twitter:image real de la
            # página (`Document.image`) sin una segunda descarga — ya
            # tenemos `downloaded` en memoria.
            try:
                ...
                if image_url and str(image_url).strip().lower().startswith(("http://", "https://")):
                    result["image"] = str(image_url).strip()
            except Exception as meta_exc:
                logger.debug("Error extrayendo og:image de %s: %s", url, meta_exc)
    except Exception as exc:
    ...
    try:
        for future in as_completed(future_map, timeout=effective_article_timeout):
            ...
            try:
                ...
                # BLINDAJE (mismo bug que en `_fetch_article_content_and_
                # image` — ver su BLINDAJE): la imagen real que esa
                # función ahora sí extrae (og:image) nunca se leía de
                # vuelta acá, así que se perdía sin importar que la
                # extracción hubiera funcionado. Solo se asigna si el
                # resultado no traía ya una imagen propia (de otro punto
                # del pipeline).
                article_image = extracted.get("image")
                if article_image and not result.get("image"):
...
def search_topic_images(
    query: str,
    max_results: int = IMAGE_SEARCH_MAX_RESULTS,
    lang: Optional[str] = None,
    log_cb: LogCallback = None,
) -> List[Dict[str, str]]:
    """
    Búsqueda de imágenes DEDICADA, independiente del scraping de
    artículos de `search_web()`.
    ...
    las tres imágenes relacionadas al tema"): antes de esto, la única
    fuente de imágenes era incidental — el og:image que el scraping de
    `_enrich_with_full_articles` lograra sacar de la página de cada
    fuente de TEXTO (y ese scraping ni siquiera extraía la imagen hasta
    el fix de `_fetch_article_content_and_image`, ver su BLINDAJE). Eso
    hace que la tarjeta visual dependa de la suerte de que el artículo
    de texto elegido por relevancia tenga además una buena foto — para
    muchos temas (Wikipedia como única fuente, artículos sin og:image,
    fuentes bloqueadas) nunca hay ninguna. Esta función corre una
    búsqueda de imágenes real (`ddgs.images()`, mismo backend que ya usa
    el resto del módulo) por el tema en sí, no por texto de un artículo
    particular — así que siempre trae hasta `max_results` fotos
    relacionadas al tema, sin depender de qué haya encontrado la
    búsqueda de texto. Falla en silencio (lista vacía) ante cualquier
    error de red — nunca debe tumbar la búsqueda de texto, que ya
    corrió y tiene su propio resultado.
    """
    clean_q = sanitize_query(query)
    if not clean_q:
        return []
    ...
    _emit_log(
        ...
        _msg(
            lang,
            "[WEB_SEARCH] Buscando imágenes del tema...",
            "[WEB_SEARCH] Searching topic images...",
        ),
    )
    try:
        with DDGS(timeout=_effective_timeout(IMAGE_SEARCH_TIMEOUT_SECONDS)) as ddgs:
            raw_items = list(
                ddgs.images(clean_q, max_results=max(max_results * 3, 10), region=region)
            )
    except Exception as exc:
        logger.debug("🛡️ [WebSearch] Búsqueda de imágenes falló: %s", exc)
        _emit_log(
            log_cb,
            _msg(
                lang,
                f"[WEB_SEARCH] Búsqueda de imágenes falló: {_describe_exception(exc)}",
                f"[WEB_SEARCH] Image search failed: {_describe_exception(exc)}",
            ),
        )
        return []
    ...
    _emit_log(
        ...
        _msg(
            lang,
            f"[WEB_SEARCH] {len(images)} imagen(es) del tema encontrada(s).",
            f"[WEB_SEARCH] {len(images)} topic image(s) found.",
        ),
    )
    return images
