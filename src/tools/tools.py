...
# Mismo flag y mismo valor que ollama_manager.py::CREATE_NO_WINDOW — evita
# que un subprocess.run/Popen lanzado desde acá (ver
# LocalToolDispatcher._compute_system_telemetry, fallback sin psutil) abra
# su propia consola visible en un build empaquetado con console=False.
...
# Cuánto se espera tras lanzar una app GUI detectada antes de decidir si
# "arrancó bien" o "crasheó al toque". Alcanza de sobra para que Python
# levante una excepción de import (ModuleNotFoundError, p. ej. pygame no
# instalado) o un SyntaxError — eso pasa en milisegundos, mucho antes de
# este margen — sin llegar a acercarse a lo que tarda un usuario en
# efectivamente cerrar la ventana a mano.
GUI_LAUNCH_GRACE_SECONDS = 2.0

# Reconoce "python archivo.py [args...]" (y variantes: python3, py, el
# lanzador pythonw sin consola) al INICIO del comando — deliberadamente
# simple: si el modelo envuelve el comando en algo más elaborado (un
# `cd X && python Y.py`, comillas con espacios en el nombre), esto no
# matchea y el comando sigue el camino bloqueante normal (con el timeout
# por defecto de arriba como red de seguridad) — más vale no detectar un
# caso raro que detectar mal uno y lanzar desatado algo que no correspondía.
_PY_SCRIPT_CMD_RE = re.compile(
    r"^\s*(?:py|pythonw?3?)\b\s+([^\s\"']+\.pyw?)\b", re.IGNORECASE
)

# Import, en cualquier línea del archivo, de una librería que arma su
# propia ventana con un bucle de eventos bloqueante — exactamente lo que
# hace que `subprocess.run` nunca vuelva hasta que el usuario cierre la
# ventana.
