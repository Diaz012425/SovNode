...
def collect_correction_pairs(wal_path: str | Path) -> list[CorrectionPair]:
    ...
    for record in _iter_wal_records(wal_path):
        ...
        if original == corrected:
            # ES: El propio LLM de corrección pudo devolver el texto sin cambios;
            # no es una preferencia real (DPO necesita chosen != rejected).
            # EN: The correction LLM may have returned the text unchanged; not
            # a real preference (DPO needs chosen != rejected).
            continue
        ...
        # ES: Evita duplicados: varias correcciones encadenadas en el mismo turno
        # pueden repetir el mismo par.
        # EN: Avoids duplicates: several chained corrections in the same turn can
