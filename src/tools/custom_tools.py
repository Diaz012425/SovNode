...
if TYPE_CHECKING:
    # ES: Solo para chequeo de tipos — ver tools.py sobre por qué esto nunca
    # se importa en tiempo de ejecución a nivel de módulo (evita un import circular).
    # EN: Type-checking only — see tools.py for why this is never imported at
    # runtime at module level (avoids a circular import).
...
# =====================================================================
# Constantes de validación / Validation constants
# =====================================================================
_VALID_NAME_RE = re.compile(r"^[a-z][a-z0-9_]{1,49}$")
_PLACEHOLDER_RE = re.compile(r"\{([a-zA-Z_][a-zA-Z0-9_]*)\}")

# ES: Allowlist de caracteres permitidos en un valor de parámetro ya
# sustituido. \w es unicode-aware (cubre acentos/ñ); deliberadamente no se
# usa \s (incluiría \n/\r), solo espacio y tab explícitos.
# EN: Allowlist of characters permitted in an already-substituted parameter
# value. \w is unicode-aware (covers accents/ñ); \s is deliberately not used
# (it would include \n/\r) — only explicit space and tab.
...
# ES: Nombres de las herramientas integradas (tools.py); una custom tool no
# puede reusar ninguno. Es la validación temprana a nivel de archivo;
# register_custom_tools() además valida contra dispatcher.has_tool() como
# defensa en profundidad.
# EN: Built-in tool names (tools.py); a custom tool can't reuse any of them.
# This is the early, per-file validation; register_custom_tools() also
# checks dispatcher.has_tool() as defense in depth.
...
# =====================================================================
# Ubicación por defecto de custom_tools/ / Default custom_tools/ location
# =====================================================================
...
# =====================================================================
# Carga y validación de specs / Loading and validating specs
# =====================================================================
def _validate_and_build_spec(
    ...
    # ES: Los placeholders del template y los parámetros declarados deben
    # coincidir exactamente — un desajuste casi siempre es un error de
    # tipeo, y más vale rechazar el archivo en la carga que dejar un
    # placeholder sin validar en tiempo de ejecución.
    # EN: The template's placeholders and declared parameters must match
    # exactly — a mismatch is almost always a typo, and it's safer to
    # reject the file at load time than leave a placeholder unvalidated at runtime.
...
def load_custom_tool_specs(directory: Optional[Path] = None) -> List[CustomToolSpec]:
    ...
    for json_path in sorted(directory.glob("*.json")):
        try:
            ...
            # ES: Un JSON mal escrito en custom_tools/ nunca debe impedir que
            # el resto de la app arranque.
            # EN: A malformed JSON file in custom_tools/ must never prevent
...
# =====================================================================
# Sustitución segura y ejecución / Safe substitution and execution
# =====================================================================
...
def _make_tool_function(dispatcher: "LocalToolDispatcher", spec: CustomToolSpec) -> Callable[..., str]:
    def _custom_tool(**kwargs: Any) -> str:
        ...
        # ES: Único camino de ejecución real: la misma sandbox que run_cmd,
        # con sus mismas defensas, corriendo sobre el comando ya sustituido.
        # EN: The one real execution path: the same sandbox as run_cmd, with
        # the same defenses, running the already-substituted command.
...
def register_custom_tools(dispatcher: "LocalToolDispatcher", directory: Optional[Path] = None) -> int:
    ...
    """
    from tools import TOOLS_SCHEMA  # ES: import diferido, ver docstring del módulo / EN: deferred import, see module docstring
