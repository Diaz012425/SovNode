...
BLOCKED_NAMES: frozenset[str] = frozenset({
    ...
    # Corrección CRÍTICA (auditoría v3.9): 'operator.attrgetter' /
    # 'operator.methodcaller' reciben el nombre del atributo o método a
    # acceder como STRING en tiempo de ejecución (p.ej.
    # operator.attrgetter('__class__')), no como sintaxis `.__class__`
    # literal. El visitor solo inspecciona nodos ast.Attribute/ast.Name
    # -- una constante de cadena es un ast.Constant, invisible para el
    # chequeo de dunders. Sin este bloqueo, una cadena de gadgets
    # ('__class__' -> '__base__' -> '__subclasses__()') llega a
    # cualquier clase cargada en el proceso (incluida
    # subprocess.Popen/os._wrap_close aunque el script nunca las
    # importe) y desde ahí a '__init__.__globals__[\"os\"].system(...)'
    # -- RCE completo evadiendo por igual la capa 1 (AST) y la lista
    # blanca de módulos, porque 'operator' es un módulo de cálculo
    # legítimo y permanece permitido.
...
class SecurityASTVisitor(ast.NodeVisitor):
    ...
    def visit_ImportFrom(self, node: ast.ImportFrom) -> None:
        ...
        # Corrección (auditoría v3.9): `from operator import attrgetter
        # as X` nunca vuelve a mencionar el texto "attrgetter" en el
        # resto del script -- de ahí en más solo aparece "X" -- así que
        # el chequeo de visit_Name/visit_Attribute contra BLOCKED_NAMES,
        # que compara el identificador tal como aparece escrito, no lo
        # detecta (el renombre con `as` lo evade). Se corrige comparando
        # el nombre ORIGINAL importado (`alias.name`), antes de
        # cualquier renombre, contra BLOCKED_NAMES.
        for alias in node.names:
            if alias.name in BLOCKED_NAMES:
                self.violations.append(
                    f"Importación restringida: "
...
# Corrección CRÍTICA (auditoría v3.9): defensa en profundidad (capa 3)
# para el gadget 'operator.attrgetter'/'methodcaller' cerrado en la capa
# 1 (SecurityASTVisitor.BLOCKED_NAMES). Si un chequeo AST futuro llegara
# a tener un hueco no previsto hoy, el modulo real se entrega aqui ya
# sin estos dos atributos, asi que ni siquiera existe la superficie para
# explotarlos -- consistente con la filosofia de "tres capas
# independientes" que documenta este archivo.
...
def _guarded_import(name, globals=None, locals=None, fromlist=(), level=0):
    ...
    for dangerous_attr in _DANGEROUS_MODULE_ATTRS.get(base, ()):
        if hasattr(module, dangerous_attr):
            try:
                delattr(module, dangerous_attr)
        ...
        try:
            ...
            if sys.platform == "win32":
                job_handle = _apply_windows_memory_limit(
                    process._handle, self.MAX_MEMORY_BYTES
                )
