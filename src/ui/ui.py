...
class AutoResizingTextBrowser(QTextBrowser):
    ...
    def __init__(self, parent: Optional[QWidget] = None) -> None:
        ...
        # BLINDAJE (pedido explícito del usuario, 2026-09-15 -- "en la
        # captura se ve como los codigos no tienen cuadro de texto
        # especial de tipo script"): `setMarkdown()` ya convierte
        # `` `código` `` (backtick simple) a fuente monoespaciada, pero
        # Qt no le agrega ningún fondo por default -- se leía casi
        # confundido con el texto normal. `self.setStyleSheet(...)` de
        # arriba es el QSS del WIDGET (fondo/borde del QTextBrowser en
        # sí); los elementos HTML DENTRO del documento renderizado
        # (`<code>`, `<h1>`, etc.) se estilan aparte, con
        # `QTextDocument.setDefaultStyleSheet()` -- nunca se llamaba.
        # LIMITACIÓN CONOCIDA: Qt soporta `background-color` para
        # corridas de texto en línea, pero NO `border-radius`/`padding`
        # ahí (sí en elementos de bloque) -- se omiten a propósito en
        # vez de dejar código muerto que Qt ignoraría igual.
        self.document().setDefaultStyleSheet(
            "code { background-color: #2A2E3A; color: #E6E8EC; "
