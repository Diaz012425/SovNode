...
# Delimitador de apertura/cierre LaTeX (`\(` `\)` `\[` `\]`) que sobrevivió
# a TODA la extracción de arriba: es residuo de un par mal formado por el
# modelo (ver `_MIXED_DELIMITER_PATTERNS`). Su expresión ya se extrajo o
# se descartó; lo único que queda colgando es el `backslash+paréntesis`.
# Se degrada a paréntesis/corchete pelado — mismo criterio que el barrido
# de etiquetas de razonamiento huérfanas en orchestrator.py: ante un
# fragmento de sintaxis que ya no puede cumplir su función, quitarlo es
# mejor que mostrarlo crudo.
...
#   \lvert \rvert \lVert \rVert — barras de valor absoluto / norma del
#       paquete amsmath. ESTA versión de mathtext no las conoce (medido,
#       video 2026-09-02, respuesta de Schrödinger: `\lvert\psi(x)\rvert
#       ^2` fallaba el render y quedaba como texto crudo). `\vert`/`|` sí
#       renderiza — se mapean a la barra pelada; `\lVert`/`\rVert` a `\|`
#       (norma doble, sí soportada).
...
def extract_equations_as_placeholders(
    ...
    # Barrido final de delimitadores LaTeX huérfanos (`\(` `\)` `\[` `\]`)
    # que quedaron colgando tras un par mal formado — ver
    # `_ORPHAN_MATH_DELIM_RE`. No toca los que ya fueron enmascarados como
    # placeholder (son texto inerte ahora) ni el contenido de un span de
    # código inline (todavía enmascarado en este punto).
