...
# Raíz real del proyecto (donde vive logo.ico), calculada UNA vez desde
# la ubicación de este propio archivo — no desde el cwd del proceso.
# `sovnode_qt.py` vive en src/ui/, así que subir 2 niveles llega a la
# raíz (…/MonolitoPersonal).
...
# Familia tipográfica propia de la app (rediseño visual pedido por el
# usuario - "el diseño actual grita mucho que está hecho con IA"). Antes
# no se fijaba NINGUNA fuente propia en toda la UI, salvo Consolas para
# bloques de código/terminal: todo el texto corría con la fuente por
# defecto de Qt/Windows (Segoe UI) sin ninguna decisión de identidad
# detrás - una de las señales más reconocibles de "interfaz sin diseñar".
# Inter (SIL Open Font License 1.1, libremente empaquetable con software
# de terceros) se eligió por estar pensada específicamente para pantalla
# en tamaños de UI pequeños -alta legibilidad a 11-13px, que es donde
# vive la mayoría del texto de esta app- y por traer los 4 pesos que el
# QSS ya usa vía `font-weight` (400/500/600/700) como instancias
# ESTÁTICAS reales en vez de negrita sintética sobre Regular.
_APP_FONT_FILES = (
    "Inter-Regular.otf",
    "Inter-Medium.otf",
    "Inter-SemiBold.otf",
...
I18N = {
    "Español": {
        ...
        "attach_tooltip": "Adjuntar una imagen (visión local)",
    ...
    "English": {
        ...
        "attach_tooltip": "Attach an image (local vision)",
...
# Nota (rediseño visual - "los 3 temas se sienten todos el mismo azul
# frío, sin identidad propia"): antes `accent` de los 3 temas vivía en la
# MISMA familia de azul-cian (#4C8BF5 / #7C9CFF / #88C0D0) - literalmente
# el azul por defecto de cualquier librería de componentes (Bootstrap,
# shadcn/ui...), sin ninguna decisión de marca detrás más allá de "un
# azul que combine". Ahora cada tema tiene una familia de tono realmente
# distinta para su identidad (accent/accent_soft/user/general_model):
# Cyberpunk Dark se queda en la familia fría pero la empuja a un cian
# eléctrico tipo HUD (coherente con el propio nombre del tema); OLED
# Pure Black pasa a violeta-magenta neón (un acento saturado se nota
# mucho más sobre negro puro que uno pastel); Nordic Slate abandona el
# cian -que competía con Cyberpunk- por el naranja "aurora" AUTÉNTICO de
# la paleta Nord real (nord12, #D08770), dando el único contraste
# cálido-sobre-frío de los tres temas. Las señales de estado
# (success/warning/danger) se dejan intactas en los 3 -verde/ámbar/rojo
# convencional- porque esas son semánticas de seguridad, no de marca:
# tocarlas rompería la intuición de "rojo = error" sin aportar nada a la
# identidad visual.
THEMES = {
    "Cyberpunk Dark": {
        "bg": "#0E1117",
...
# Colores del tema activo, disponibles a nivel de módulo para todo lo que
# se construye "sobre la marcha" y no recibe el diccionario de tema por
# parámetro (MessageBubble por remitente, DonationDialog, el QAction de
# "Copiar texto"...): esos widgets se crean de nuevo en cada turno/uso,
# así que leer de aquí en el momento de construirse basta para que sus
# íconos (ver icons.py) seteen el color correcto sin tener que enhebrar
# el tema como parámetro por cada constructor. MainWindow._apply_theme()
# la actualiza cada vez que el usuario cambia de tema en el combo.
_ACTIVE_THEME: dict[str, str] = dict(THEMES["Cyberpunk Dark"])

# Nombre de familia real que Qt le asignó a Inter (ver `load_app_fonts`),
# fijado UNA vez en main() tras crear el QApplication. `build_style()` lo
# recibe como parámetro en cada llamada para no depender de que todo el
# código de temas conozca de dónde viene - si `load_app_fonts()` nunca se
# llamó (p. ej. un test que importa este módulo sin pasar por main()),
# se queda en este valor por defecto, que es exactamente lo que Qt
# intentaría de todos modos si "Inter" no estuviera cargada.
...
# Voces de Windows/SAPI5 con acento de ESPAÑA (es-ES) — se prueban
# PRIMERO para español, por pedido explícito del usuario ("una voz en
# español con acento español"). Helena/Laura/Pablo son las voces es-ES
# que Windows instala habitualmente; Sabina/Raúl son es-MX (respaldo
# solo si no hay ninguna es-ES instalada, mejor eso que voz en inglés).
...
# Modo voz — palanca de tamaño de modelo + caché de instancia
# (2026-09-08, pedido explícito del usuario: "mejora el modo voz").
#
# ANTES: VoiceRecorderWorker.run() instanciaba un WhisperModel NUEVO en
# CADA grabación ("tiny", cargado desde disco a memoria una vez por
# click de mic) — carga completa pagada una y otra vez sin necesidad,
# ya que el modelo no cambia entre grabaciones. Con el modelo cacheado
# a nivel de módulo, la carga se paga UNA sola vez por sesión (o CERO
# veces si `_warm_up_whisper_model` ya la disparó en segundo plano al
# abrir la ventana — ver MainWindow.__init__) y toda grabación
# posterior arranca a transcribir de inmediato.
#
# De paso, el tamaño default sube de "tiny" (39M parámetros) a "base"
# (74M): "tiny" se equivoca seguido en dictado real (sobre todo con
# ruido de fondo o acentos), y "base" sigue siendo liviano en CPU con
# `compute_type="int8"` — no es la elección que más tarda, es la más
# chica que ya no era la mejor relación precisión/latencia disponible.
# Overrideable por entorno (mismo patrón que SOVNODE_OLLAMA_NUM_THREAD
# en orchestrator.py) para quien prefiera "tiny" (más rápido) o
# "small"/"medium" (más preciso, más lento) según su hardware.
...
class StreamTurnWorker(QThread):
    ...
    def __init__(
        ...
        # Ruta de la imagen adjuntada con el botón "+" (ver icons.py
        # "attach" y MainWindow._send_message) — se pasa tal cual a
        # `run_turn`, que la usa para entrar al carril VISION dedicado
        # (moondream) cuando no es None.
...
# Nota (2026-09-02, pedido explícito del usuario): la clase
# ReflectionWorker (mensajes espontáneos — un timer de 20 min que podía
# hacerle escribir a SovNode sin que nadie le hablara) se eliminó por
# completo. Nunca se usó en la práctica ("realmente nunca sirvió de
# utilidad") y quedaba apagada por defecto — ver el botón 💭 que existía
# junto al campo de texto, y Orchestrator.generate_spontaneous_reflection
# (también eliminado). No confundir con el watcher de Workspaces de más
# abajo: ese indexa archivos en segundo plano, nunca escribe mensajes.

# Intervalo entre pasadas de escaneo del watcher de Workspaces. No hay
# nada mágico en 15s: es un punto medio entre "un archivo editado se
# refleja razonablemente rápido en el RAG" y "no quemar CPU haciendo
# os.walk() sobre carpetas potencialmente grandes todo el tiempo". Con el
# debounce de dos pasadas de WorkspaceScanner, un cambio real tarda entre
# 1x y 2x este intervalo en confirmarse — nunca menos, porque esa es la
# garantía que evita indexar a mitad de una escritura.
...
class WorkspaceWatcherWorker(QThread):
    ...
    file_indexed = pyqtSignal(str, int)  # (ruta, cantidad de chunks)
    file_removed = pyqtSignal(str, int)  # (ruta, cantidad de chunks retirados)
    ...
    def run(self) -> None:
        while self._running:
            ...
            # Espera larga trozada en pasos cortos: permite que stop()
            # (llamado desde el hilo de UI, p. ej. al cerrar la ventana)
            # surta efecto en <1s en vez de hasta 15s.
...
class ModelDownloadDialog(QDialog):
    ...
    # mistral-nemo-12b-uncensored retirado a pedido del usuario en su
    # momento; reemplazado ahora por el nuevo default general (ver abajo)
    # a pedido explícito del usuario (2026-09-03: "el mejor modelo sin
    # censura... + el mejor coder que lo acompañe").
    # Sugerencias de descarga, no un selector: el modelo general lo fija
    # Orchestrator.RESPONSE_MODEL (huihui_ai/qwen2.5-abliterate:7b-instruct)
    # y el coder Orchestrator.CODER_MODEL (qwen2.5-coder:7b) — ver
    # `_select_model_for_decision`. nomic-embed-text es el de embeddings;
    # el resto son alternativas que el usuario puede querer a mano.
    COMMON_TAGS = (
...
class MessageBubble(QWidget):
    ...
    def __init__(
        ...
        # lo normal.
        # El ícono (ver icons.py) reemplaza el emoji que llevaba antes
        # cada rótulo pegado al frente del texto ("🧑 TÚ", "⚠️ ERROR",
        # "🛠️ SOVNODE..."). Se colorea con _ACTIVE_THEME (ver su
        # definición) en vez de recibir el tema como parámetro del
        # constructor - MessageBubble se crea de nuevo en cada turno, así
        # que leer el tema activo en este momento ya basta.
        ...
        # Miniatura de la imagen adjuntada con el botón "+" (ver icons.py
        # "attach" / _send_message) — solo aplica al globo del USUARIO: la
        # respuesta del modelo de visión es texto, igual que cualquier
        # otra respuesta de SovNode. Se agrega ANTES del texto (como
        # cualquier chat clásico: foto arriba, caption/pregunta abajo).
        # Fallo silencioso si el archivo ya no existe o no es una imagen
        # válida (p. ej. el usuario lo borró después de enviarlo) — un
        # globo sin miniatura sigue siendo un globo funcional, nunca debe
        # tumbar el render del chat.
...
class MainWindow(QMainWindow):
    ...
    def __init__(self) -> None:
        ...
        # Modo voz — mitad TTS: la síntesis de voz (TTSWorker) ya estaba
        # completamente escrita (selección de voz ES/EN vía SAPI5,
        # detención al enviar un mensaje nuevo o al cerrar la app — ver
        # _send_message/_quit_application, ambos YA llamaban a
        # _stop_tts() antes de este cambio) pero nada disparaba
        # _play_tts() nunca: cero botones conectados. `self.tts_enabled`
        # es el toggle que faltaba — apagado por defecto (opt-in, para
        # que el asistente no empiece a hablar solo sin que el usuario
        # lo haya pedido) y se prende/apaga con `self.tts_toggle_button`
        # (ver _create_ui / _toggle_tts_enabled).
        ...
        # Ejecutor de tareas en segundo plano para trabajo que NO es un
        # turno de chat (p. ej. indexar un archivo soltado en la ventana
        # — ver _on_file_dropped_for_indexing): async_executor.py ya
        # existía en el proyecto (QThreadPool + QRunnable) pero no tenía
        # ningún caller real todavía.
        self._async_executor = AsyncExecutor()

        # "Workspaces": carpetas vigiladas en segundo plano (ver
        # WorkspaceWatcherWorker / workspace_watcher.WorkspaceScanner más
        # arriba, y la card "Workspaces" del panel de configuración más
        # abajo). El QThread se crea acá pero recién se arranca (`.start()`)
        # al agregar la PRIMERA carpeta (`_on_add_workspace_clicked`) — no
        # tiene sentido correr un loop de escaneo desde el segundo 0 de la
        # app si el usuario nunca usa esta función.
        ...
        # Pestañas de chat: cada sesión guarda su propio transcript
        # (lista de ChatEntry) para poder cambiar de pestaña sin perder
        # lo que se venía conversando en las demás. LIMITACIÓN CONOCIDA
        # (ver _activate_session): la memoria de corto plazo del
        # Orchestrator (memory_graph + vector_rag) es UNA sola para todo
        # el proceso, así que cambiar de pestaña resetea esa memoria del
        # modelo aunque el transcript VISUAL de cada pestaña se conserve.
        ...
        # Panel de métricas (VRAM + tok/s) dentro de la terminal — ver
        # _refresh_metrics(). sys_optimizer.get_system_telemetry() ya
        # trae su propia caché TTL de 5s, así que sondear cada 3s desde
        # acá casi siempre pega ese caché en vez de spawnear
        # PowerShell/nvidia-smi de nuevo.
        self.metrics_timer = QTimer(self)
        self.metrics_timer.timeout.connect(self._refresh_metrics)
        self.metrics_timer.start(3000)

        # Autoguardado periódico de los índices FAISS (workspace/longterm,
        # ver Orchestrator.save_vector_indices en orchestrator.py). El
        # guardado en _quit_application() cubre un cierre ORDENADO, pero
        # no un crash o un corte de luz — sin esto, la memoria sintética
        # generada (o los archivos de workspace reindexados) entre el
        # último cierre ordenado y un crash se perdería igual. 10 min es
        # un compromiso: cada .save() reescribe el índice COMPLETO (ver
        # LocalVectorRAG.save, sin journaling incremental), así que un
        # intervalo más corto gastaría IO de disco sin necesidad real.
    ...
    def _quit_application(self) -> None:
        ...
        if hasattr(self, "orchestrator") and self.orchestrator is not None:
            try:
                ...
                # El cierre de la app nunca debe fallar por esto — en el
                # peor caso, la próxima sesión arranca sin la memoria
                # sintética/de workspace que no llegó a guardarse (ver
                # Orchestrator.save_vector_indices en orchestrator.py).
                pass
    ...
    def _on_intent_changed(self, icon: str, msg: str) -> None:
        ...
        # El scroll solo baja solo al enviar (_send_message ya lo hizo hace
        # un instante); el indicador de "pensando" no arrastra la vista.
    ...
    def _on_web_results_ready(self, data: dict) -> None:
        ...
        # Se inserta la tarjeta justo arriba de la respuesta en curso, sin
        # mover el scroll: puede llegar varios segundos después del envío,
        # cuando el usuario quizá ya subió a leer (el auto-scroll solo
        # actúa al enviar).
    ...
    def _create_ui(self) -> None:
        ...
        # PANEL DE CONFIGURACIÓN (antes: panel lateral SIEMPRE visible).
        # Rediseño pedido explícitamente: solo quedan visibles arriba el
        # botón "Apoyar el proyecto" y el botón de config (⚙) al lado —
        # todo lo que antes vivía fijo en esta barra lateral (tema,
        # idioma, estado del nodo, descarga de modelo, exportar) pasa a
        # un panel oculto por defecto que ⚙ muestra/oculta (ver
        # _toggle_config_panel). Se mantiene como QFrame normal dentro de
        # main_layout (no un popover flotante) — layout de Qt se encarga
        # de acomodar el resto de la ventana solo con setVisible().
        sidebar = QFrame()
        ...
        # Estado del Nodo
        #
        # El estado (online/offline/verificando) vivía como un emoji de
        # semáforo ("🟢"/"🔴"/"🟡") pegado al frente del propio texto -
        # otra vez la misma señal de "sin diseñar" (el color depende de
        # la fuente de emoji del SO, no del tema activo). Ahora es un
        # punto de color real (`status_dot`, un QLabel de 9x9 con
        # border-radius:50% pintado vía setStyleSheet en
        # _set_node_status_color) que sigue el acento/success/danger del
        # tema activo, en vez de un glifo de color fijo.
        status_card = QFrame()
        ...
        # (qwen2.5:7b — ver Orchestrator.RESPONSE_MODEL). Se conserva SOLO
        ...
        # Workspaces — carpetas que SovNode vigila en segundo plano
        # (WorkspaceWatcherWorker) e indexa/reindexa automáticamente en
        # Orchestrator.workspace_vector_rag, para que fetch_hybrid_context()
        # las tenga disponibles como conocimiento persistente entre
        # sesiones/pestañas de chat (a diferencia de un archivo soltado a
        # mano, que solo se indexaba una vez).
        #
        # La lista de carpetas SÍ persiste entre reinicios de la app —
        # guardada en sovnode_memory.db (MemoryGraph.workspace_folders) y
        # restaurada por _load_persisted_workspaces() en __init__. Nota:
        # solo se persisten las RUTAS, no el índice vectorial en sí —
        # workspace_vector_rag vive en memoria de proceso (FAISS) y arranca
        # vacío en cada inicio, así que WorkspaceWatcherWorker tiene que
        # re-escanear y re-embeber el contenido de cada carpeta restaurada
        # antes de que vuelva a ser recuperable por RAG (con el debounce de
        # 2 pasadas de WorkspaceScanner, no es instantáneo).
        ...
        sidebar_layout.addWidget(workspaces_card)

        sidebar_layout.addStretch()

        # btn_new_chat y btn_donate se construyen más abajo, en la fila de
        # pestañas y en el header respectivamente — "nuevo chat" ahora es
        # el "+" de la barra de pestañas (ver más abajo), y "Apoyar el
        # proyecto" pasó a ser uno de los DOS botones que quedan siempre
        # visibles arriba, junto al de configuración.
        ...
        # Oculto por defecto — ⚙ lo muestra/oculta (_toggle_config_panel).
        ...
        # FILA DE PESTAÑAS DE CHAT — reemplaza al único botón "Nueva
        # conversación" que antes vivía fijo en la barra lateral: ahora
        # cada pestaña es una conversación independiente en pantalla (ver
        # _new_session_dict/_activate_session/_on_new_tab_clicked/
        # _on_tab_close_requested). "+" crea una pestaña nueva; la (x) de
        # cada pestaña la cierra.
        ...
        # La (x) de cierre ya no es la nativa (icono rojo del SO) — cada
        # pestaña recibe su propio botón vía _add_tab_close_button(), que
        # se estiliza por QSS junto al resto de la UI.
        ...
        # "+" pegado al final de las pestañas (patrón de navegador), no
        # flotando solo en el extremo derecho. El stretch final absorbe el
        # espacio sobrante cuando hay pocas pestañas; si se llenan, el
        # QTabBar encoge y saca sus flechas de desplazamiento.
        ...
        # Los DOS únicos botones que quedan siempre visibles arriba, pedido
        # explícito del rediseño: "Apoyar el proyecto" (antes al fondo de
        # la barra lateral) + "⚙ Config" al lado, que muestra/oculta el
        # panel con todo lo que antes vivía fijo en esa barra lateral.
        ...
        # El ícono real (engranaje dibujado en icons.py) se pinta en
        # _refresh_themed_icons, llamado por _apply_theme() justo
        # después de que _create_ui() termine de construir todos los
        # botones - reemplaza el glifo "⚙" que dependía de la fuente de
        # emoji del SO y no podía teñirse del color del tema.
        ...
        # Indicador de estado (texto con segundos transcurridos).
        # La barra de progreso indeterminada que iba acá se sacó (pedido
        # explícito del usuario, 2026-09-08: "elimina esta barra de
        # carga... queda feo" — el modo indeterminado de QProgressBar en
        # este tema se ve como dos segmentos sueltos, no como una barra
        # animada prolija). El texto con los segundos transcurridos
        # (`self.processing_label`, ver _on_send_message/_on_turn_
        # completed) se mantiene: es la señal real de que hay un turno
        # en curso, sin el elemento visual que quedaba feo.
        ...
        # Vista previa del archivo adjuntado (botón "+", ver más abajo y
        # Orchestrator.run_turn/carril VISION) — miniatura + nombre + "x"
        # para sacarlo antes de enviar. Oculta por defecto
        # (setVisible(False)): solo aparece tras elegir una imagen en
        # _open_attach_file_dialog, y vuelve a ocultarse al enviar o al
        # tocar la "x" (_clear_attached_image). Vive en su propio
        # QWidget entre el indicador de progreso y la fila de entrada,
        # mismo lugar donde un chat clásico (WhatsApp/Telegram/ChatGPT)
        # muestra el adjunto pendiente antes de escribir el mensaje.
        self._attached_image_path: Optional[str] = None
        # (rediseño visual, pedido del usuario "podemos mejorar la
        # interfaz en diseño?"): antes esta vista previa era una fila de
        # miniatura+nombre+"x" flotando directo sobre el fondo del chat,
        # sin ningún fondo/borde propio - el único elemento "agrupado" de
        # toda la interfaz sin la tarjeta que separa visualmente todo lo
        # demás (sidebarCard, userCard, la insignia del header...). Ahora
        # vive dentro de una tarjeta chica (`attachmentPreviewCard`, ver
        # build_style) que abraza su contenido en vez de ocupar todo el
        # ancho de la fila - un "chip" de adjunto pendiente, como en
        # cualquier chat clásico. El wrapper exterior conserva los mismos
        # márgenes (24px) que la fila de entrada de debajo para que el
        # chip quede alineado con el botón "+" que tiene justo encima.
        ...
        self.attachment_remove_btn.clicked.connect(self._clear_attached_image)
        ...
        # Botón "+" clásico para adjuntar una imagen (pedido explícito del
        # usuario, 2026-09-08: "diseña el clasico + para adjuntar
        # archivos... que el diseño sea parecido a la interfaz" — mismo
        # tamaño/estilo que mic_button/tts_toggle_button: QPushButton
        # 48x48, "secondaryButton", ícono propio de icons.py en vez de un
        # emoji). Va PRIMERO en la fila (a la izquierda del campo de
        # texto), el lugar clásico de un botón de adjuntar en un chat.
        ...
        # RAG (chunking AST): un archivo soltado en la ventana ya se
        # inyectaba en el prompt de ESTE turno (ver PromptTextEdit.
        # dropEvent), pero se perdía apenas terminaba — esta conexión
        # además lo indexa en vector_rag para turnos FUTUROS. La señal
        # existía sin ningún listener conectado.
        self.input_field.file_dropped.connect(self._on_file_dropped_for_indexing)

        # Íconos reales (mic / globo, ver icons.py) pintados por
        # _refresh_themed_icons tras _create_ui() - reemplazan "🎙️" y
        # "🌐", el mismo caso que btn_config más arriba.
        ...
        # Modo voz — mitad TTS (2026-09-08): botón toggle para leer en
        # voz alta las respuestas del asistente. Apagado por defecto
        # (opt-in) — ver self.tts_enabled/_toggle_tts_enabled.
        ...
        # El botón manual de "forzar búsqueda web" (🌐) se sacó (pedido
        # explícito del usuario, sesión de rediseño): la decisión de
        # buscar en internet pasa a ser 100% autónoma del lado del
        # router determinista (SignalTag.WEB_SEARCH_INTENT vía
        # IntentRouter.classify - ver router.py), sin que el usuario
        # tenga que tildar nada turno a turno. `force_web_search` como
        # PARÁMETRO sigue existiendo en Orchestrator.run_turn/
        # process_turn (lo sigue usando _should_force_web_search para la
        # excepción de FACTUAL_ENUMERATION) - lo que se elimina es
        # exclusivamente el control manual de la UI que lo encendía a
        # pedido del usuario.

        # objectName propio ("stopButton", ver build_style) en vez de
        # "secondaryButton" (rediseño visual, pedido del usuario "podemos
        # mejorar la interfaz en diseño?"): antes "Detener" se veía
        # IDÉNTICO a mic/tts/adjuntar -mismo gris neutro, solo el ícono
        # llevaba el color de peligro (ver _refresh_themed_icons)-, pese
        # a ser la única acción que cancela algo en curso. Un tinte sutil
        # de "danger" en fondo/borde lo hace reconocible de un vistazo
        # mientras el turno está corriendo, sin gritar tanto como un rojo
        # sólido.
        ...
        # Panel de métricas en tiempo real (VRAM vía sys_optimizer.py,
        # tok/s vía Orchestrator._log_generation_perf) — ver
        # _refresh_metrics(), llamado por self.metrics_timer cada 3s
        # mientras este panel esté visible. Antes ambos datos solo salían
        # por logger.info()/consola de texto plano, nunca en la interfaz.
    ...
    def _on_lang_changed(self, lang_name: str) -> None:
        ...
        # btn_new_chat es el "+" compacto de la barra de pestañas (ver
        # _create_ui) — se traduce el tooltip, no el texto del botón.
        self.btn_new_chat.setToolTip(tr["btn_new_chat_tooltip"])
        self.btn_config.setToolTip(tr["btn_config_tooltip"])
        ...
        # Pestaña(s) sin título propio todavía (aún no se les mandó ningún
        # mensaje) — re-renderiza su rótulo genérico en el idioma nuevo.
    ...
    def _toggle_terminal(self) -> None:
        ...
        if self._terminal_visible:
            # Refresco inmediato al abrir — si no, la etiqueta queda en
            # blanco hasta el próximo tick de self.metrics_timer (≤3s).
    ...
    # -----------------------------------------------------------------
    # Pestañas de chat — ver _chat_sessions/_active_session_index en
    # __init__ y la nota de LIMITACIÓN CONOCIDA en _activate_session.
    # -----------------------------------------------------------------
    ...
    def _on_tab_close_requested(self, index: int) -> None:
        ...
        if closing_active_tab:
            # Invalida el índice ANTES de tocar la lista: si no, el
            # próximo _on_tab_bar_changed (Qt reubica el foco solo al
            # cerrar la pestaña activa) podría snapshotear el transcript
            # saliente sobre la sesión que quedó corrida a este mismo
            # índice tras el del().
        ...
        if not closing_active_tab:
            return  # la pestaña activa sigue siendo la misma sesión — nada que re-renderizar

        if not self._chat_sessions:
            # Nunca quedarse sin ninguna pestaña.
            self._chat_sessions.append(self._new_session_dict())
            self.chat_tab_bar.blockSignals(True)
            self.chat_tab_bar.addTab(I18N[self._current_lang]["tab_new_chat_title"])
            self._add_tab_close_button(0)
    ...
    def _on_file_dropped_for_indexing(self, file_path: str) -> None:
        ...
        # Ruta ABSOLUTA completa como source_id (no solo el basename):
        # Orchestrator.index_document_for_rag() usa este id para borrar
        # e idempotentemente reemplazar los chunks previos de ESTE mismo
        # archivo en re-indexados futuros (ver rag_faiss.remove_source).
        # Con solo el nombre, "config.py" soltado desde dos carpetas
        # distintas compartiría source_id y cada uno pisaría al otro.
    ...
    def _sync_tool_sandbox_root(self) -> None:
        ...
        la lista, vuelve a `os.getcwd()` — el comportamiento original.
    ...
    def _load_persisted_workspaces(self) -> None:
        ...
        for folder in folders:
            if self._workspace_scanner.add_root(folder):
                self.workspaces_list.addItem(QListWidgetItem(folder))
    ...
    def _on_add_workspace_clicked(self) -> None:
        ...
        try:
            ...
            # No debe impedir que el workspace funcione EN ESTA sesión —
            # en el peor caso, no sobrevive al próximo reinicio.
    ...
    def _open_attach_file_dialog(self) -> None:
        ...
        nativo de archivos filtrado a imágenes y guarda la ruta elegida en
        `self._attached_image_path` — `_send_message` la lee al enviar y
        se la pasa a `StreamTurnWorker`/`Orchestrator.run_turn` (carril
        VISION, `image_path=...`). No se copia ni se sube el archivo a
        ningún lado: SovNode es 100% local, así que `run_turn` lo abre
        directo desde esta ruta del disco del usuario para codificarlo en
        base64 y mandárselo a Ollama (localhost).
        """
        tr = I18N.get(self._current_lang, I18N["Español"])
        ...
        self.attachment_thumb_label.setPixmap(pixmap)
        self.attachment_name_label.setText(os.path.basename(file_path))
    ...
    def _clear_attached_image(self) -> None:
        ...
        junto con la imagen y arrepentirse solo de la imagen."""
    ...
    def _send_message(self) -> None:
        ...
        # Con una imagen adjunta (botón "+") alcanza con enviarla sola,
        # sin texto — `Orchestrator._build_vision_prompt` ya cubre ese
        # caso con una pregunta genérica de descripción. Sin adjunto, se
        # mantiene la guarda de siempre (nada que enviar).
        ...
        # Único punto donde el chat baja solo: al enviar. Se re-ancla la
        # vista al fondo y se fuerza el scroll (aunque el usuario estuviera
        # leyendo más arriba) para que su mensaje recién enviado y el
        # arranque de la respuesta queden a la vista. A partir de aquí el
        # streaming ya no mueve el scroll (ver _scroll_to_bottom).
    ...
    def _on_turn_completed(self, trace: Any, error_msg: str) -> None:
        ...
        if error_msg:
            ...
            # Modo voz — mitad TTS que faltaba conectar (2026-09-08):
            # `_play_tts`/`_stop_tts` ya existían completos (esta última
            # ya se llamaba desde `_send_message`/`_quit_application`),
            # pero nada disparaba la lectura en sí. Se lee el texto
            # CANÓNICO (`final_text`, post-limpieza de razonamiento
            # fugado) y solo si el usuario prendió el toggle — nunca por
            # defecto, para no sorprender a nadie con el asistente
            # hablando sin haberlo pedido.
            if self.tts_enabled:
    ...
    def _apply_theme(self, theme_name: str) -> None:
        if theme_name in THEMES:
            ...
            # `_ACTIVE_THEME` es lo que leen los widgets que se
            # construyen "sobre la marcha" (burbujas de mensaje nuevas,
            # el diálogo de donación, el menú de "Copiar texto") para
            # teñir sus íconos - ver la nota en su definición más arriba.
            _ACTIVE_THEME.clear()
            _ACTIVE_THEME.update(theme)

            # Los botones que YA existen en pantalla (construidos una
            # sola vez en _init_ui) necesitan que se les vuelva a pintar
            # el ícono a mano: un QIcon es un pixmap ya rasterizado, no
            # sigue el QSS del tema como sí lo hace el texto.
            self._refresh_themed_icons(theme)

            # Idem para el punto de estado del nodo (ver
            # _set_node_status_color) - no es un ícono, pero por la
            # misma razón (color fijado en el momento de pintarlo, no
            # vía QSS) necesita este mismo empujón manual al cambiar de
            # tema.
    ...
    def _refresh_themed_icons(self, theme: dict[str, str]) -> None:
        ...
        # El tamaño de icons.icon(..., size) y el de setIconSize() tienen
        # que COINCIDIR: icons.icon() devuelve un único pixmap rasterizado
        # a `size` (a 2x internamente para HiDPI, ver icons.py), no un
        # ícono "escalable" - pedirle a Qt un setIconSize() distinto solo
        # fuerza un reescalado extra sobre un pixmap ya rasterizado, y se
        # nota (bordes borrosos) en un set de íconos pensado para verse
        # nítido a un tamaño exacto.
        _ICON_SIZE_PX = 16
    ...
    def _scroll_to_bottom(self, force: bool = False) -> None:
        ...
        # Los globos renderizan Markdown/LaTeX de forma diferida, así que
        # su altura final llega uno o dos ciclos de evento después: un
        # último reintento asegura que el envío quede pegado al fondo.
        if force:
    ...
    def _flush_stream_buffer(self) -> None:
        ...
        # Antes aquí se llamaba a _scroll_to_bottom() en cada tick de
        # render (~50 ms): con el anclaje al fondo activo tras el envío,
        # eso arrastraba la vista durante toda la generación y no dejaba
        # leer la respuesta desde arriba. El scroll automático ahora solo
        # ocurre al enviar; para alcanzar el final hay botón flotante.
        #
        # Sí hace falta re-evaluar el anclaje: como la vista NO baja pero
        # el contenido sigue creciendo, el usuario se aleja del fondo aun
        # sin tocar el scroll. Sin esto, _pinned_to_bottom se quedaría en
        # True toda la generación y el botón flotante "ir al final" nunca
        # aparecería mientras se lee desde arriba.
        if hasattr(self, "chat_scroll") and self.chat_scroll:
    ...
    def _add_bubble(
        ...
        # `image_path` (adjunto del botón "+", ver _send_message) NO se
        # persiste en ChatEntry/_replay_session_entries — solo vive en el
        # MessageBubble de esta sesión. Al cambiar de pestaña y volver, el
        # mensaje del usuario se re-renderiza sin la miniatura (queda el
        # texto). Aceptado como límite conocido: la sesión completa de
        # replay de pestañas ya excede el pedido original ("implementa
        # Moondream y el botón +"); persistir el adjunto en el snapshot de
        # pestaña es trabajo aparte si hiciera falta más adelante.
    ...
    def _toggle_voice_recording(self) -> None:
        if not self._is_recording_voice:
            ...
            # El ícono de mic se tiñe del color "danger" del tema activo
            # en vez del rojo #F2555A fijo que llevaba antes (ver
            # _refresh_themed_icons) - con los 3 temas ahora usando
            # familias de acento distintas (idea 1 del rediseño), un
            # rojo hardcodeado seguía siendo el mismo en los 3, cosa que
            # ya no combina con el resto de la UI recoloreada.
            ...
            self._is_recording_voice = False
            # Estado transitorio (transcribiendo) - se deja como texto
            # simple en vez de un ícono propio: dura poco, y un
            # reloj-de-arena de línea añadiría un ícono más al set solo
            # para un par de segundos de espera.
...
def main() -> int:
    ...
    # Tipografía propia (rediseño visual - ver load_app_fonts): tiene que
    # cargarse DESPUÉS de crear QApplication (QFontDatabase necesita una
    # QGuiApplication ya viva) y ANTES de construir MainWindow (para que
    # el primer build_style() ya reciba el nombre de familia real, en vez
    # de tener que reconstruir el stylesheet una segunda vez tras
    # cargarla). Si falla, `family` cae en la fuente por defecto del
    # sistema - degradación silenciosa, ver la nota de load_app_fonts().
