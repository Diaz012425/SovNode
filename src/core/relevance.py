...
_PRECISE_FACT_QUERY_RE: Pattern[str] = re.compile(
    ...
    # como quedo el mundial 2026" no matcheaba NINGUNA palabra de la lista
    # de arriba, así que este gate daba False y ninguno de los
    # verificadores anti-alucinación (find_unsupported_scores,
    # find_unsupported_victory_claims) llegaba a ejecutarse. "cómo
    # quedó/terminó/salió" es una construcción coloquial de "cómo terminó
    # esto" -- pide un desenlace puntual igual que "quién ganó", así que
    # se agrega sin ampliar el gate a frases genéricas como "detalles de"
    # (eso sí dispararía de más en consultas no relacionadas con un
    # desenlace). Nota: la consulta de seguimiento "dime los detalles de
    # ese mundial" -- sin ninguna palabra de desenlace propia -- queda
    # fuera de este fix; es una limitación conocida.
    r"|c[oó]mo\s+(?:qued[oó]|termin[oó]|sali[oó])\b"
    r"|\bturn(?:ed)?\s+out\b",
    re.IGNORECASE,
)
