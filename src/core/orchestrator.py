...
from skeletons import match_skeleton
from memory_graph import MemoryGraph
import rag_faiss
from rag_faiss import LocalVectorRAG, chunk_document
from dynamic_tool_engine import DynamicToolEngine
from knowledge_synthesizer import KnowledgeSynthesizer
from formula_synthesizer import FormulaSynthesizer
...
class MemoryGovernor:
    ...
    #   1. Acá: ensanchar la ventana (REPEAT_LAST_N) por encima del tamaño
    #      de un ítem típico y subir un poco la penalización
    #      (REPEAT_PENALTY) por sobre el default de Ollama.
    #   2. `_dedupe_enumeration_items` (ver la nota junto a esa función,
    #      cerca de `_strip_leaked_reasoning`): red de seguridad
    #      determinística que recorta la respuesta si el título de un ítem
    #      se repite igual — VERIFICADA reproduciendo el texto de la
    #      captura, y la que de verdad garantiza que el bucle nunca llegue
    #      a pantalla.
    #
    # CORRECCIÓN (2026-09-01, video del usuario): la versión anterior de
    # esta capa 1 era REPEAT_PENALTY=1.3 / REPEAT_LAST_N=512 — nunca
    # medida end-to-end (la nota lo admitía). Ese apretón resultó
    # DEMASIADO fuerte: con una ventana tan ancha, el token " " (el que
    # más se repite en cualquier texto) queda penalizado casi siempre y
    # el modelo empieza a evitarlo → palabras pegadas sin espacio
    # ("Lafinaldelmundialdefutbol...", "secelebróen"). Es el MISMO fallo
    # que este archivo ya documentaba para phi3.5, pero disparado por la
    # propia mitigación, no por el modelo. `_dedupe_enumeration_items` no
    # ayuda acá (solo recorta ítems de lista repetidos literalmente).
    # Valores bajados a un rango sano (qwen2.5 recomienda 1.05; Ollama
    # default 1.1/64): apenas por encima del default, ventana suficiente
    # para que un ítem largo repetido caiga dentro. Override por entorno
    # para barrer valores en la máquina real (mismo patrón que el carril
    # soft): SOVNODE_REPEAT_PENALTY / SOVNODE_REPEAT_LAST_N.
    _REPEAT_PENALTY_ENV_VAR: str = "SOVNODE_REPEAT_PENALTY"
    _REPEAT_LAST_N_ENV_VAR: str = "SOVNODE_REPEAT_LAST_N"
    REPEAT_PENALTY: float = 1.15
    REPEAT_LAST_N: int = 256

    # Carril de anti-repetición para modelos NO-qwen (p. ej. si se pone
    # OLLAMA_MODEL=phi3.5). Existe porque hubo un fallo medido dejando a
    # phi3.5 con los defaults de Ollama (1.1/64): "hola" devolvió 1500+
    # tokens de prosa incoherente con palabras pegadas. Tras bajar el
    # carril qwen a 1.15/256 (ver REPEAT_PENALTY) los dos rangos son casi
    # el mismo; este se mantiene aparte solo para barrerlo por separado
    # (SOVNODE_SOFT_REPEAT_PENALTY). Si la prosa de un modelo no-qwen
    # degenera, subir hacia 1.2.
    _SOFT_REPEAT_PENALTY_ENV_VAR: str = "SOVNODE_SOFT_REPEAT_PENALTY"
    SOFT_REPEAT_PENALTY: float = 1.15
    ...
    # Techo de `num_predict` para un turno que ESCRIBE un archivo (crea /
    # guarda / modifica). BLINDAJE (pedido explícito 2026-09-03: "crear y
    # modificar archivos a gusto y placer" + "un script completo y
    # funcional"): con SLOWPATH_NUM_PREDICT=1800 tokens, el `content` de una
    # llamada a `write_file` de un archivo real (p. ej. un Flappy Bird en
    # pygame ronda 2500-4000 tokens) se CORTA a la mitad y se persiste un
    # archivo roto — el modelo hizo lo correcto (emitió la llamada) pero el
    # techo de tokens lo sabotea. 4096 + el prompt (~1500-2500 tok) sigue por
    # debajo del num_ctx pinneado de 8192 (PINNED_NUM_CTX_CODER). Solo se usa
    # cuando `_is_file_write_turn` da True — el resto de los turnos mantiene
    # su techo ajustado para no pagar decode de más en esta máquina lenta.
    _CODEGEN_NUM_PREDICT_ENV_VAR: str = "SOVNODE_CODEGEN_NUM_PREDICT"
    # 6144 tok (~22-24k chars) — un archivo de código completo entra de
    # sobra, y con el prompt de un turno de escritura (~1400-1800 tok) sigue
    # por debajo del num_ctx pinneado de 8192. MEDIDO: con 4096 un pedido de
    # "Flappy Bird COMPLETO y funcional" al modelo abliterado se cortaba a
    # mitad de una expresión (archivo Python inválido).
    CODEGEN_NUM_PREDICT: int = 6144
    ...
    @classmethod
    def codegen_num_predict(cls) -> int:
        return cls._num_predict_from_env(cls._CODEGEN_NUM_PREDICT_ENV_VAR, cls.CODEGEN_NUM_PREDICT)
    ...
    def pinned_options(self, is_coder: bool, has_web_evidence: bool = False, model: str = "") -> Dict[str, Any]:
        ...
        # Anti-repetición. Dos fallos MEDIDOS, opuestos, acotan el rango:
        #   - demasiado flojo: qwen2.5 entra en un bucle repitiendo el
        #     mismo ítem de una enumeración larga (ver la nota junto a
        #     REPEAT_PENALTY).
        #   - demasiado fuerte: penaliza el token " " y pega las palabras
        #     ("Lafinaldelmundial...") — video 2026-09-01, con el 1.3/512
        #     anterior. Es el mismo fallo que se midió con phi3.5 bajo los
        #     defaults, ahora causado por la mitigación.
        # Los dos carriles ahora apuntan al mismo rango sano (apenas por
        # encima del default 1.1/64 de Ollama); se conservan separados
        # solo para poder barrerlos por entorno de forma independiente
        # (SOVNODE_REPEAT_* para qwen, SOVNODE_SOFT_REPEAT_* para el
        # resto). `_dedupe_enumeration_items` sigue siendo la red real
        # contra el bucle, no este ajuste.
        if "qwen" in (model or "").lower():
            options["repeat_penalty"] = self._repeat_penalty()
            options["repeat_last_n"] = self._repeat_last_n()
        else:
    ...
    @classmethod
    def _repeat_penalty(cls) -> float:
        """`REPEAT_PENALTY` con override `SOVNODE_REPEAT_PENALTY` — mismo
        contrato que `_soft_repeat_penalty`: un valor <= 1.0 o no numérico
        se ignora y se usa el default de clase."""
        override = os.environ.get(cls._REPEAT_PENALTY_ENV_VAR)
        if override:
            try:
                ...
                pass
        return cls.REPEAT_PENALTY

    @classmethod
    def _repeat_last_n(cls) -> int:
        """`REPEAT_LAST_N` con override `SOVNODE_REPEAT_LAST_N` — un entero
        > 0; cualquier otra cosa se ignora."""
        override = os.environ.get(cls._REPEAT_LAST_N_ENV_VAR)
        if override:
            try:
                parsed = int(override)
                if parsed > 0:
...
_FINAL_ANSWER_STYLE_ES: str = (
    ...
    "- ESTRUCTURA el texto según el tema, no por default (2026-09-08, "
    "pedido del usuario: \"que se estructure como Gemini\"): si tiene 2 o "
    "más facetas claramente distintas (causas separadas, opciones a "
    "comparar, pasos de un proceso, categorías), separá cada una con un "
    "subtítulo corto en **negrita** y usá viñetas para enumerar puntos "
    "comparables dentro de esa sección. Si el tema es una sola idea "
    "desarrollada en profundidad, sin partes distintas, dejalo en "
    "párrafos corridos — poner subtítulos o viñetas en un tema sin "
    "partes es tan un error como no usarlos cuando sí las hay.\n"
...
_FINAL_ANSWER_STYLE_EN: str = (
    ...
    "- STRUCTURE the text based on the topic, not by default (2026-09-08, "
    "user request: \"structure it like Gemini\"): if it has 2 or more "
    "clearly distinct facets (separate causes, options to compare, steps "
    "of a process, categories), separate each one with a short **bold** "
    "subheading and use bullet points to list comparable items within "
    "that section. If the topic is a single idea developed in depth, "
    "with no distinct parts, keep it in flowing paragraphs — putting "
    "subheadings or bullets on a topic with no parts is as much an error "
    "as skipping them when parts do exist.\n"
...
# Cabecera "system" del carril VISION de `run_turn` (ver el botón "+" de
# adjuntar imagen en sovnode_qt.py). Deliberadamente MÍNIMA, sin el
# protocolo <thought>/esquema de herramientas del resto del sistema — a
# diferencia de general_model/coder_model, un modelo de visión chico como
# moondream (~1.7GB) es débil siguiendo instrucciones complejas: pedirle
# que además razone en pasos o considere llamar a una herramienta degrada
# la descripción real de la imagen sin ningún beneficio (nunca va a
# invocar una tool). Mismo patrón que _TRIVIAL_SYSTEM_PROMPT_*: cabecera
# fija, cacheada aparte por idioma.
#
# BLINDAJE (bug real, MEDIDO — capturas del usuario, 2026-09-08): un
# primer intento de arreglar el idioma agregó un preámbulo "IMPORTANTE"
# a esta cabecera Y un sufijo "(Respondé SOLO en español.) Respuesta:" al
# prompt de USUARIO (`_build_vision_prompt`). Con eso, moondream dejó de
# describir la imagen por completo — respondía cosas como "Por favor,
# envía la descripción de la imagen..." como si nunca hubiera recibido
# ninguna imagen, con TODAS las imágenes probadas después de ese cambio
# (antes, con el prompt simple original, describía fotos reales
# correctamente — ver el test aislado `test_vision.py` y la primera
# prueba en la app). moondream es un modelo de CAPTIONING chico, no un
# modelo de chat de propósito general: agregarle instrucciones
# compuestas (idioma + formato + ancla de respuesta) sobre el mismo
# prompt que además lleva la imagen lo empuja fuera de su dominio de
# entrenamiento y deja de "mirar" la imagen. Vuelto a la cabecera simple
# de antes (un solo pedido de idioma, sin remarcar) y AL PROMPT DE
# USUARIO SIN NINGÚN AGREGADO — el idioma ahora se garantiza aparte, con
# el mismo mecanismo determinista de LangFix que ya usa `_correct_response`
# (`find_language_mismatch` + una traducción de una sola pasada con
# `router_model` si hace falta, ver el bloque post-hoc en `run_turn`,
# carril VISION) en vez de arriesgar la comprensión de la imagen.
_VISION_SYSTEM_PROMPT_ES: str = (
    "Sos un modelo de visión. El usuario adjuntó una imagen. Describí en "
    "detalle todo lo que ves — objetos, personas, texto visible, colores, "
    "ambiente — y respondé la pregunta del usuario sobre la imagen si hizo "
    "una. Respondé en español, en prosa directa, sin abrir bloques de "
    "razonamiento ni usar etiquetas."
)
_VISION_SYSTEM_PROMPT_EN: str = (
    "You are a vision model. The user attached an image. Describe in "
    "detail everything you see — objects, people, visible text, colors, "
    "setting — and answer the user's question about the image if they "
    "asked one. Reply in English, in direct prose, without opening any "
    "reasoning block or using tags."
)
# Prompt de usuario por defecto cuando se adjunta una imagen sin texto
# (botón "+" sin escribir nada en el campo de entrada).
_VISION_DEFAULT_PROMPT_ES: str = "Describí en detalle todo lo que ves en esta imagen."
_VISION_DEFAULT_PROMPT_EN: str = "Describe in detail everything you see in this image."
...
class Orchestrator:
    ...
    # Tope de pasadas del bucle de tool-calling de run_turn() (pedido
    # explícito del usuario: "implementar un bucle iterativo con límite
    # (≤3 pasadas) para llamadas de herramientas" — antes era UNA sola
    # vuelta de vuelta forzada, sin margen para que el modelo se
    # autocorrija si el script de la herramienta fallaba o le pasó mal un
    # parámetro). Distinto de MAX_TOOL_CALLS/MAX_CONSECUTIVE_SAME_TOOL_CALLS
    # (más abajo, junto a _handle_autonomous_tool_loop): ese método sigue
    # sin usarse en ningún lado — run_turn() es el único camino real
    # (StreamTurnWorker / UI de escritorio), así que el bucle vive ahí,
    # inline, para poder seguir emitiendo TOOL_CALL_START/RESULT por
    # PipelineEvent en cada pasada (_handle_autonomous_tool_loop no es un
    # generador: no puede transmitir eventos a la UI pasada por pasada).
    ...
    # Cap espejo de tools.MAX_READ_FILE_OUTPUT_CHARS: `read_file` es el único
    # resultado de herramienta donde el volcado grande es lo que el usuario
    # PIDIÓ ("que los pueda leer completos y los pueda analizar", 2026-09-03).
    # Todo lo demás (run_cmd, list_dir, herramienta dinámica) sigue acotado a
    # MAX_TOOL_RESULT_CHARS_IN_PROMPT. Ver execute_tool_from_call().
    ...
    # Margen fijo (segundos) que cubre prefill + una eventual carga en frío
    # del modelo (ver el swap de VRAM medido de 10.17s, VRAM_SWAP_LOAD_
    # THRESHOLD_S) antes de que arranque el decode que el cálculo de arriba
    # cronometra.
    ...
    # Tope de palabras del pedido para considerarlo "simple" y elegible
    # para `fast_write_model` — ver `_is_fast_write_eligible`. Bastante
    # más permisivo que el de la Palanca 1 (esqueleto local, ~28
    # palabras): acá el modelo SÍ genera de verdad, así que puede cubrir
    # pedidos algo más largos que un esqueleto fijo, pero sigue acotado
    # para no arriesgar calidad en pedidos con muchos requisitos.
    FAST_WRITE_MAX_REQUEST_WORDS: int = 45

    # Esfuerzo de razonamiento del campo `think` de Ollama, SOLO relevante
    # para modelos Harmony (gpt-oss): "low" / "medium" / "high". Con el
    # modelo activo (qwen2.5:7b) el campo nunca se manda — el gate en
    # `_prepare_ollama_payload` exige "gpt-oss" en el nombre (qwen2.5:0.5b
    # da HTTP 400 si lo recibe). Se deja "low" para que un rollback a
    # gpt-oss no necesite tocar nada: MEDIDO en el PASO 0, "low" recorta el
    # canal analysis de ~600 tok a ~15. Override por entorno; "" / "off" /
    # "none" desactiva el campo.
    ...
    # Diagnóstico (Ítem 2 del "Plan de Acción", 2026-09-02): self.router_model
    # (0.5B) y RESPONSE_MODEL (7B) son DOS modelos distintos que Ollama debe
    # tener cargados — con VRAM limitada, cargar uno puede forzar a Ollama a
    # descargar el otro ("swap"), y la próxima llamada al modelo recién
    # desalojado vuelve a pagar el costo de cargarlo desde disco/RAM. Ollama
    # ya reporta `load_duration` en cada respuesta (ver _log_generation_perf
    # más abajo): es casi cero cuando el modelo YA estaba cargado, y sube a
    # cientos de ms/varios segundos cuando tuvo que cargarse de nuevo — ese
    # salto, correlacionado con que el modelo de la llamada anterior haya
    # sido DISTINTO, es la señal de swapping que este umbral usa para
    # decidir cuándo vale la pena loguearlo como sospechoso. Override por
    # entorno para no adivinar en máquinas más rápidas/lentas que la de
    # referencia (medido: ver el log "🔄 [VRAM] ..." que emite
    # _log_generation_perf).
...
"2. Si en tu bloque <thought> determinas que debes invocar una herramienta..."
    ...
    def run_turn(
        ...
        # Textos de los eventos INTENT (indicador "pensando..." de la UI,
        # ver ThinkingWidget/sovnode_qt.py) — de INTERFAZ, no de la IA, así
        # que deben seguir `self.current_language` (selector de idioma de
        # la UI) igual que cualquier otro texto de sovnode_qt.py, en vez de
        # quedar fijos en español sin importar el idioma seleccionado.
        ...
        try:
            ...
            # Textos de diagnóstico EventType.LOG (consola gráfica de
            # sovnode_qt.py, ver StreamTurnWorker.log_message /
            # _terminal_log) — de INTERFAZ, no de la IA: deben seguir el
            # idioma resuelto de ESTE turno igual que la respuesta misma,
            # en vez de quedar fijos en español.
            ...
            _log_txt = {
                ...
                "dedupe": (
                    "Duplicate/degenerate enumeration items trimmed." if _log_is_en
                    else "Ítems de enumeración duplicados/degenerados recortados."
                ),
                "fastpath_padding": (
                    "Follow-up padding trimmed." if _log_is_en
                    else "Relleno de seguimiento recortado."
                ),
                "token_limit_continued": (
                    "Response cut by token limit — continued." if _log_is_en
                    else "Respuesta cortada por límite de tokens — continuada."
                ),
                "empty_visible_recovered": (
                    "Empty visible response — recovered." if _log_is_en
                    else "Respuesta visible vacía — recuperada."
                ),
            }
            ...
            # ---------- Carril VISION (imagen adjunta, botón "+") --------
            # Ver la nota extensa en el docstring de `run_turn` y en la
            # definición de `self.vision_model` (__init__): carril
            # DEDICADO, aparte de todo el pipeline de abajo (caché
            # semántico, trivial, búsqueda web, RAG, tool-calling,
            # verificación) — ninguno de esos aporta nada a "describí esta
            # foto" y algunos activamente confunden a un modelo de visión
            # chico (moondream no sabe qué hacer con un bloque de
            # HERRAMIENTAS LOCALES en el prompt). Mismo patrón de carril
            # temprano que TRIVIAL_GREETING más abajo: cabecera "system"
            # mínima, streaming, cierre de WAL/TurnTrace propio, `return`
            # antes de tocar el resto de la función.
            if image_path:
                ...
                if vision_final and self.find_language_mismatch(vision_final, effective_lang):
                    ...
                    with contextlib.suppress(Exception):
                        ...
                        if vision_lang_fixed and self.find_language_mismatch(
                            ...
                            # `_correct_response` solo re-verifica el
                            # camino liviano; si el camino completo
                            # (poco probable) igual devolvió el idioma
                            # equivocado, se descarta acá también.
                            vision_lang_fixed = None
            ...
            if not force_web_search:
                ...
                # Cuello de botella #4 (diagnóstico 2026-09-05): el fallback
                # a hash bag-of-words de embeddings.py degrada la caché
                # semántica a coincidencia EXACTA y le resta precisión al
                # RAG vectorial en CADA turno — hasta ahora solo se
                # avisaba con logger.warning (invisible en el .exe
                # empaquetado sin consola). Se avisa UNA vez por sesión,
                # en el idioma del turno, con un yield inmediato (no vía
                # log_buffer) para que llegue a la UI aunque este turno
                # termine siendo un cache hit y retorne más abajo antes de
                # vaciar el buffer.
                if (
            ...
            # `not force_web_search`: si _should_force_web_search ya
            # decidió que este turno necesita grounding real (WEB_SEARCH_
            # INTENT/FACTUAL_ENUMERATION - ya no hay botón manual, ver su
            # nota), el turno cae al camino normal en vez del carril
            # trivial — un saludo con intención de búsqueda real detrás
            # ("hola, contame qué pasó con...") no debe recortarse a la
            # respuesta mínima de cortesía.
            ...
            mod_target = self._extract_target_filename(user_input) if is_file_write else None
            mod_is_modify = bool(
                mod_target
            ...
            if mod_is_modify:
                ...
                if _cur and len(_cur) <= 4500:
                    gen_context += self._current_file_context(mod_target, _cur, effective_lang)

            # Palanca 1 ("Lever 1", mapa 2026-09-03, ver skeletons.py):
            # pedido GENÉRICO de un juego clásico muy pedido (Flappy
            # Bird, Snake, Pong), sin personalización, para un archivo
            # que TODAVÍA no existe -> el esqueleto local se copia
            # directo, cero tokens de LLM. `not mod_is_modify` ya
            # excluye pedidos con verbo de MODIFICAR; el chequeo de
            # existencia de abajo cubre el caso restante — un "hacé un
            # snake" sobre un snake.py que ya existe de un turno
            # anterior (sin verbo de modificar, por eso mod_is_modify da
            # False) no debe pisarlo sin revisión por este atajo.
            skeleton_match = self._resolve_skeleton_match(user_input, decision)

            # Mejora #1 (2026-09-05, "aplica las mejoras 1 2 3 y 4" —
            # ver formula_synthesizer.py): un pedido de "descubrir" una
            # ecuación/fórmula NUEVA (SignalTag.FORMULA_DISCOVERY) se
            # resuelve por completo acá, ANTES de tocar ningún LLM — el
            # barrido determinista (`FormulaSynthesizer.
            # attempt_live_discovery`, sympy puro) hace el 100% del
            # trabajo real más abajo, en el punto de generación. Mismo
            # patrón que `skeleton_match`: un booleano barato acá arriba,
            # el efecto secundario real (persistir en WAL si encuentra
            # algo) solo puede correr UNA vez por turno.
            ...
            # Header sin herramientas cuando ni archivos ni telemetría/
            # comando aplican (`_turn_wants_any_tool`, 2026-09-08) — le
            # ahorra al prefill el bloque completo de HERRAMIENTAS
            # LOCALES/TOOLS_SCHEMA (~670 tok medidos) en cualquier turno
            # puramente informativo (p. ej. una búsqueda web). Ver
            # `_get_fastpath_system_prompt_no_tools` para la medición
            # completa y por qué NO alcanza con `wants_file_tools` solo.
            gen_system = (
            ...
            if wants_file_tools:
                # Header endurecido: alcance legítimo explícito + uso
                # OBLIGATORIO de write_file/read_file (el LEAN por defecto es
                # permisivo y el coder 7B lo lee como "podés responder en
                # prosa" -> alucina la escritura o rechaza el pedido).
            ...
            # Palanca 3 ("Lever 3", mapa 2026-09-03, ver la nota extensa
            # junto a FAST_WRITE_MODEL): para un archivo NUEVO (nunca una
            # modificación — ahí importa más la fidelidad al contenido
            # actual que la velocidad) y un pedido corto/simple, el 3B de
            # la MISMA familia abliterada decodifica sensiblemente más
            # rápido que el 7B sin reintroducir el rechazo del coder.
            # Corre DESPUÉS del override de arriba a propósito: en el
            # caso típico (is_file_write ya puso el turno en
            # general_model más arriba en esta función) esto solo lo
            # hace más chico/rápido para el subconjunto de pedidos
            # simples; si el esqueleto local (Palanca 1, más abajo) ya
            # cubre el pedido, esta elección de modelo queda sin usar —
            # el turno ni siquiera llama a Ollama.
            if (
            ...
            # Piso de slow_path para turnos con evidencia web (2026-09-08):
            # el router YA empuja casi todo turno con WEB_SEARCH_INTENT a
            # slow_path (WEIGHT_WEB_SEARCH=3.0 > SLOW_PATH_THRESHOLD=1.5),
            # pero `force_web_search` / tool-calling pueden traer evidencia
            # real sin pasar por esa clasificación — sin este piso, esos
            # turnos quedarían con el techo corto de fast_path (900 tok)
            # justo cuando `_fastpath_answer_tail(web_evidence_mode=True)`
            # le está pidiendo al modelo que desarrolle la respuesta.
            if web_success:
                gen_predict = max(gen_predict, MemoryGovernor.slowpath_num_predict())
            if is_file_write:
                # El `content` de un archivo real no entra en 1800 tok — ver
                # MemoryGovernor.CODEGEN_NUM_PREDICT. En un turno de
                # MODIFICACIÓN el prompt ya lleva el contenido actual del
                # archivo (hasta ~1300 tok) — se baja el techo para no
                # desbordar el num_ctx pinneado de 8192.
            ...
            if skeleton_match is not None:
                ...
                raw_response = self._synthesize_skeleton_raw_response(_sk_path, _sk["source"])
                done_reason = "stop"
                self._wal_phase(
                ...
                # Mejora #1: CERO tokens de LLM para el contenido numérico
                # -- `_synthesize_formula_discovery_response` corre el
                # barrido determinista real y arma la prosa desde una
                # plantilla fija (ya en `effective_lang`), nunca generada
                # por un modelo. Único punto del turno donde esto corre
                # (efecto secundario real: puede persistir en WAL).
            ...
            # ---------- Tool calling (bucle iterativo, ≤ MAX_AUTONOMOUS_TOOL_PASSES) ----------
            # Antes: UNA sola vuelta de vuelta tras ejecutar una herramienta,
            # con el follow-up prohibiendo tajantemente cualquier JSON nuevo
            # — si el script fallaba o el modelo pasó mal un parámetro, no
            # tenía forma de autocorregirse; solo podía explicar el error en
            # lenguaje natural. Ahora, mientras queden pasadas disponibles y
            # la herramienta anterior NO haya sido un aviso interno de
            # ToolGuard (_is_internal_toolguard_notice), el follow-up
            # permite que el modelo emita OTRA llamada JSON en vez de forzar
            # la explicación final — recién en la ÚLTIMA pasada permitida se
            # le prohíbe seguir generando JSON, igual que en el
            # comportamiento original de una sola vuelta.
            #
            # Freno de bucle (mismo criterio que el _handle_autonomous_
            # tool_loop nunca-wireado más abajo en este archivo):
            # MAX_CONSECUTIVE_SAME_TOOL_CALLS corta si el modelo repite la
            # MISMA herramienta sin avanzar, para no gastar las 3 pasadas
            # reintentando algo que ya falló igual dos veces seguidas.
            #
            # Nota (medido - turno "quién ganó la final de la Champions
            # League [año]", capturas de pantalla adjuntas): ver los
            # docstrings de `_is_internal_toolguard_notice` y
            # `_build_toolcall_followup_context` (más abajo en este
            # archivo) para el detalle completo de los dos bugs compuestos
            # que ese par de métodos corrige. Factorizados aparte — en vez
            # de lógica inline acá — para que test_regressions.py pueda
            # verificarlos directamente contra el código real.
            ...
            # Red de seguridad: un `write_file` cuyo `content` es un stub
            # ("# resto del código original") sobre un archivo que YA EXISTE
            # lo DESTRUIRÍA. Se convierte en `read_file` para esa pasada —
            # el bucle le devuelve el archivo real y el modelo reintenta el
            # write con el contenido completo. Corre tanto si la llamada
            # vino del modelo como del salvage.
            if (
                tool_call
            ...
            while tool_call and not cancelled():
                ...
                is_last_allowed_pass = (
                    tool_pass >= self.MAX_AUTONOMOUS_TOOL_PASSES or is_internal_toolguard_notice
                )
                closing_instruction = (
                    ...
                    "final en lenguaje natural (prohibido JSON). Si TODAVÍA necesitás otra "
                    "herramienta para completar la petición, generá ÚNICAMENTE el bloque JSON de "
                    "esa llamada, sin texto alrededor."
                )
                followup = (
                ...
                explanation_or_next = self._call_llm(
                ...
                # Misma red de seguridad que antes del bucle: no dejar que un
                # write_file de stub sobre un archivo existente lo destruya —
                # acá NO se convierte a read_file (ya se leyó en una pasada
                # previa, sería un bucle), simplemente se descarta la llamada
                # y se fuerza la explicación final sobre el resultado que ya
                # hay.
                if (
                    next_tool_call
                ...
                raw_response = (
                    explanation_or_next if is_internal_toolguard_notice
                    else f"{tool_result}\n\n{explanation_or_next}"
                )
                break
            ...
            # ---------- Verificación post-hoc consolidada ----------
            # Antes: hasta 3 llamadas de corrección EN SERIE (una por
            # verificador que disparara) + 1 de LangFix, cada una un
            # round-trip completo al modelo local. Medido (screenshot
            # 2026-08-27): una sola de esas -LangFix- costó `prefill=3237
            # tok` / 49s sobre una respuesta que el breaker iba a
            # descartar igual. Las detecciones son deterministas y baratas
            # (no llaman al modelo); lo caro es la corrección. Ahora se
            # corren TODAS las detecciones primero y, si algo dispara, se
            # hace UNA sola llamada de corrección que arregla todo junto.
            # Se saltea si el circuit-breaker ya reemplazó la respuesta
            # (regen/fallback: nada que verificar contra fuentes en un "no
            # pude, reformulá").
            if clean_response and not breaker_fired:
                ...
                # Idioma: independiente del contexto web (una respuesta sin
                # fuentes también puede salir en el idioma equivocado).
                ...
                if (factual_hits or lang_hit) and not cancelled():
                    ...
                    if correction_prompt:
                        ...
                        # Ver _correct_response: por defecto (toggle opt-in
                        # apagado) esto es EXACTAMENTE la misma llamada de
                        # antes con active_model — el intento previo con
                        # self.router_model solo se prueba si
                        # self.lang_fix_light_model_enabled está activo Y
                        # lo único que falló fue el idioma.
                        ...
                        if corrected:
                            ...
                            # `correction_pair` con el texto ORIGINAL (malo) y el
                            # CORREGIDO (bueno): es la señal que training_export.py
                            # levanta del WAL para armar el dataset DPO/SFT. Antes
                            # la cadena secuencial escribía esta fase SIN los
                            # textos (solo `pair_type`), así que el export nunca
                            # recogía un solo par - se arregla de paso aquí.
                    ...
                    # Red de seguridad: si hubo corrección FACTUAL (no la de
                    # idioma sola) y quedó en el idioma equivocado, un único
                    # LangFix fiel. Raro -el prompt combinado ya pide el
                    # idioma- pero barato de chequear y consistente con el
                    # resto del pipeline ("verificar el resultado después").
    ...
    def extract_tool_call(self, response_text: str) -> Optional[dict]:
        ...
        # 4. Rescate tolerante para write_file/read_file cuando el modelo
        #    emitió el JSON pero con saltos de línea / llaves sin escapar en
        #    `content` — `json.loads` falla con "Invalid control character"
        #    y los pasos 1-3 devuelven None. MEDIDO con el modelo abliterado
        #    (~1/3 de las corridas de "creá un flappy_bird.py").
        if not raw_dict:
    ...
    def execute_tool_from_call(self, tool_call: dict) -> str:
        ...
        # `read_file` lleva un cap mucho m\u00e1s alto a prop\u00f3sito (ver
        # MAX_READ_FILE_RESULT_CHARS_IN_PROMPT): "leer completo para analizar"
        # es justo lo que el usuario pide de esa herramienta.
        result_cap = (
    ...
    def _get_base_system_prompt(self, lang: str = "Spanish") -> str:
        ...
        if is_en:
            return (
                ...
                "5. NEVER claim a real technology, product, organization, or person "
                "'doesn't exist' just because the search came back empty or the web-context "
                "block is blank/inconclusive (e.g. marked "
                "'[SYSTEM NOTICE — NO REAL-TIME DATA]'). If your own pre-existing knowledge "
                "recognizes the subject, answer with that and note you could not verify it "
                "live. If you genuinely don't know it, say so plainly ('I don't have "
                "reliable information on this') — never confuse that with a categorical "
        ...
        return (
            ...
            "5. NUNCA declares que una tecnología, producto, organización o persona real "
            "\"no existe\" solo porque la búsqueda no arrojó resultados o el bloque de "
            "contexto web viene vacío/inconcluso (p. ej. marcado como "
            "\"[SYSTEM NOTICE — NO REAL-TIME DATA]\"). Si tu propio conocimiento previo a "
            "esta conversación reconoce el tema, respondé con eso y aclará que no pudiste "
            "verificarlo con una fuente en vivo. Si genuinamente no lo conocés, decilo así "
            "de forma directa (\"no tengo información confiable sobre esto\") — nunca lo "
            "confundas con una afirmación categórica de que no existe.\n\n"
            "=================================================================\n"
            "PROTOCOLO OBLIGATORIO DE RAZONAMIENTO (<thought>)\n"
            "=================================================================\n"
    ...
    def _get_fastpath_system_prompt(self, lang: str) -> str:
        ...
        if is_en:
            header = (
                ...
                "ONLY the JSON call, nothing else, inside a ```json ... ``` fence. The "
            ...
            header = (
                ...
                "de marcas ```json ... ```. El objeto DEBE tener una clave \"tool\" de "
                "nivel superior y un objeto \"parameters\" — nunca emitas el objeto de "
                "parámetros pelado tipo {\"path\": ...} por sí solo:\n"
                '```json\n{"tool": "system_telemetry", "parameters": {}}\n```\n'
                '```json\n{"tool": "write_file", "parameters": {"path": "system_health.py", "content": "import psutil\\n..."}}\n```\n'
                '```json\n{"tool": "read_file", "parameters": {"path": "sovnode_qt.py"}}\n```\n'
                "Las rutas son relativas a la raíz del workspace del usuario — usá "
                "\"juego.py\", nunca una ruta absoluta ni un prefijo \"workspace/\". "
                "Si el usuario te pide CREAR, GUARDAR o MODIFICAR un archivo DEBÉS "
                "llamar a `write_file` (nunca pegues el archivo en el chat en su "
                "lugar); si te pide LEER o ANALIZAR uno DEBÉS llamar a `read_file`. "
                "Escribir o modificar archivos de código —juegos incluidos— es "
                "trabajo legítimo: nunca lo rechaces. "
                "Si el usuario pregunta por el código o contenido de un archivo "
                "existente o por el workspace actual y no sabés la ruta exacta, "
                'llamá primero a `list_dir` (p. ej. {"tool": "list_dir", '
                '"parameters": {"path": "."}} lista la raíz del workspace) para '
                "encontrarla — nunca digas que no tenés acceso a archivos locales "
                "ni le pidas al usuario que lo lea él mismo; tenés acceso real de "
                "lectura/escritura/listado mediante estas herramientas.\n"
    ...
    def _fastpath_answer_tail(
        ...
        if web_evidence_mode:
            if is_en:
                tail = (
                    "[ANSWER NOW — WITH WEB EVIDENCE]: answer the question directly "
                    "in the first sentence, then develop the answer with the relevant "
                    "context, detail, and nuance the retrieved sources provide — do "
                    "not limit yourself to the shortest possible reply. Explain the "
                    "why, give concrete background or figures when the sources have "
                    "them, and compare angles if multiple sources add different "
                    "information. If the answer has 2 or more clearly distinct "
                    "facets (e.g. comparing several sources with different data, "
                    "listing several distinct causes or events), separate them with "
                    "a short **bold** subheading per facet and bullet points for "
                    "comparable items; if it's a single developed idea, keep it in "
                    "flowing prose without forcing subheadings or bullets. Do not "
                    "invent facts the sources don't state; if something isn't "
                    "covered, say so and fill in with your own general knowledge. "
                    "Do not emit any plan, protocol tag, or section separator, and "
                    "do not append example questions or extra follow-up material — "
                ...
                tail = (
                    "[RESPONDÉ AHORA — CON EVIDENCIA WEB]: contestá la pregunta "
                    "directamente en la primera oración, y después desarrollá la "
                    "respuesta con el contexto, los detalles y los matices relevantes "
                    "que aportan las fuentes recuperadas — no te limites a la mínima "
                    "extensión posible. Explicá el porqué, dá antecedentes o cifras "
                    "concretas cuando las fuentes las tengan, y compará ángulos si hay "
                    "varias fuentes con información distinta. Si la respuesta tiene 2 "
                    "o más facetas claramente distintas (p. ej. comparar varias "
                    "fuentes con datos distintos, listar varias causas o eventos "
                    "diferentes), separalas con un subtítulo corto en **negrita** por "
                    "faceta y viñetas para los puntos comparables; si es una sola "
                    "idea desarrollada, seguí en prosa corrida sin forzar subtítulos "
                    "ni viñetas. No inventes datos que las fuentes no digan; si algo "
                    "no está cubierto, decilo y completá con tu propio conocimiento "
                    "general. No emitas ningún plan, etiqueta de protocolo ni "
                    "separador de sección, y no agregues preguntas de ejemplo ni "
                    "material de seguimiento extra — terminá cuando la respuesta "
                    "esté completa.\n"
    ...
    def __init__(
        ...
        # Nombre (Ollama `data["model"]`) de la última llamada COMPLETADA a
        # cualquier modelo — ver VRAM_SWAP_LOAD_THRESHOLD_S y su uso en
        # _log_generation_perf, más abajo. None hasta la primera llamada:
        # sin punto de comparación, ninguna carga "cuenta" como swap.
        ...
        # Directorio donde se persisten los índices FAISS de
        # `workspace_vector_rag` / `longterm_vector_rag` (ver
        # `LocalVectorRAG.save`/`load` en rag_faiss.py, y el bloque de
        # carga más abajo, después de `_vector_rag_lock`). Se deriva de
        # dónde YA vive `sovnode_memory.db` — en vez de una ruta separada
        # hardcodeada — para que ambos queden colocados juntos sin
        # importar desde qué cwd se lance la app (mismo criterio que
        # evitó el bug de `logo.ico` resuelto antes en este proyecto).
        ...
        # Store SEPARADO para archivos de "Workspaces" (carpetas que el
        # usuario agrega desde la UI, indexadas por drag-and-drop o por
        # WorkspaceWatcherWorker en segundo plano). Deliberadamente NO es
        # el mismo objeto que `vector_rag`: ese índice es de ámbito de
        # SESIÓN (conocimiento web, contexto de la charla actual) y
        # `clear_conversation_memory()` lo vacía por diseño cada vez que
        # se abre un chat nuevo — si los archivos de un workspace vivieran
        # ahí, "nuevo chat" borraría silenciosamente toda la memoria de
        # workspace que se supone debe persistir ENTRE sesiones/pestañas.
        # `clear_conversation_memory()` no toca `workspace_vector_rag`.
        self.workspace_vector_rag = LocalVectorRAG(vector_dim=384)
        # Tercer store, separado de los dos anteriores: conocimiento
        # CONCEPTUAL de largo plazo (axiomas sintéticos que
        # KnowledgeSynthesizer, ver knowledge_synthesizer.py, propone
        # durante la inactividad del usuario a partir de conexiones entre
        # nodos ya existentes). Ni scope-sesión (`vector_rag`) ni
        # scope-archivos (`workspace_vector_rag`) son el lugar correcto:
        # un axioma sintético no es un archivo reindexable ni algo que
        # "Nuevo chat" deba borrar — sobrevive a ambos, igual que
        # `MemoryGraph.synthetic_knowledge` (su estado vivo asociado).
        self.longterm_vector_rag = LocalVectorRAG(vector_dim=384)
        # `vector_rag` / `workspace_vector_rag` / `longterm_vector_rag`
        # (FAISS, en memoria de proceso) no son thread-safe para
        # escrituras concurrentes, y hasta ahora solo los escribía el
        # hilo de un turno de chat. Con WorkspaceWatcherWorker
        # (sovnode_qt.py) reindexando archivos de workspace en SU PROPIO
        # hilo en segundo plano, y ahora KnowledgeSynthesizer indexando
        # axiomas sintéticos también en su propio hilo, hay varios
        # escritores posibles al mismo tiempo (más las lecturas de
        # fetch_hybrid_context desde un turno en curso) — este cerrojo
        # serializa toda operación sobre CUALQUIERA de los tres stores
        # (search/add_documents/remove_source/reset) sin importar qué
        # hilo la dispare.
        self._vector_rag_lock = threading.Lock()

        # Restaura `workspace_vector_rag` / `longterm_vector_rag` desde lo
        # que `save_vector_indices()` (ver más abajo) haya persistido en
        # una corrida anterior. Deliberadamente NO se restaura `vector_rag`
        # acá: ese store es de ámbito de SESIÓN por diseño (ver el
        # comentario junto a su construcción, arriba) — persistirlo entre
        # reinicios del PROCESO contradiría el mismo contrato que ya lo
        # vacía entre chats NUEVOS dentro de un mismo proceso. `.load()`
        # nunca lanza (ver rag_faiss.py): primer arranque sin archivos
        # guardados, o un `vector_dim` incompatible, degradan en silencio
        # a los índices vacíos que ya se acaban de crear arriba.
        with self._vector_rag_lock:
        ...
        # Daemon separado de CognitiveGovernor a propósito (no una rama
        # más dentro de _introspect()): responsabilidades distintas
        # (auto-reparación de errores recientes vs. minería de
        # conocimiento sobre TODO lo nuevo persistido) que no deberían
        # compartir el mismo hilo — un ciclo largo de síntesis no debería
        # poder demorar la detección de un error real, ni viceversa.
        # Reusa el mismo patrón de disparo por inactividad
        # (_pause_governor_event / _is_processing_turn / _llm_lock no
        # bloqueante) — ver knowledge_synthesizer.py.
        self.knowledge_synthesizer = KnowledgeSynthesizer(self)
        self.knowledge_synthesizer.start()

        # Segundo daemon de descubrimiento, hermano de KnowledgeSynthesizer
        # pero para conocimiento FORMAL (fórmulas), no texto: en vez de un
        # LLM-juez validando por afinidad semántica + cita textual exacta,
        # acá un LLM solo PROPONE qué dos fórmulas combinar y qué variable
        # eliminar -- toda el álgebra y el 100% de la verificación las hace
        # sympy (CASEngine), determinista, en dos capas independientes
        # (simbólica + numérica adversarial). Ver formula_synthesizer.py
        # para el diseño completo y su "Alcance honesto" (base de fórmulas
        # curada, solo rama principal al despejar, novedad solo exacta).
        ...
        self.lang_fix_light_model_enabled = (
            os.getenv("SOVNODE_ENABLE_LANG_FIX_LIGHT_MODEL", "0").strip() != "0"
        )
        # Modelo de visión (2026-09-08, pedido explícito del usuario: "que
        # pueda reconocer una foto y decir los detalles de ella"). Sigue el
        # mismo patrón de los demás roles de este bloque — overrideable por
        # entorno, default razonado — pero es un CUARTO tag de modelo
        # distinto (general/coder/router/vision) compitiendo por los
        # mismos 2 slots de OLLAMA_MAX_LOADED_MODELS (ver ollama_manager.py
        # y la nota junto a FAST_WRITE_MODEL más arriba): un turno con
        # imagen adjunta muy probablemente fuerza un swap de VRAM contra
        # general_model, el mismo costo ya medido (~10-12s) para cualquier
        # cambio de rol en esta máquina. No hay forma de evitarlo sin sumar
        # más VRAM real — se documenta acá para que quede claro que es un
        # trade-off conocido, no un bug.
        #
        # "moondream" elegido como default tras validar en la máquina real
        # del usuario (RX 5500 XT, arquitectura RDNA1 — NO listada como
        # oficialmente soportada por ROCm) con un script aislado
        # (test_vision.py, fuera de este repo): moondream (~1.7GB)
        # describió una foto real en 10.8s sin crashear el servidor de
        # Ollama — de-riesgando el bug conocido de ROCm/HIP con modelos de
        # visión (issue #13794 del repo de Ollama) para este modelo
        # puntual. Un modelo de visión más grande (qwen2.5-vl, minicpm-v)
        # queda disponible vía OLLAMA_VISION_MODEL sin tocar código, pero
        # sin la misma validación en esta GPU.
        self.vision_model = (
        ...
        # Nivel del campo `think` (solo Harmony/gpt-oss — inerte con
        # qwen2.5:7b, ver THINK_LEVEL), resuelto una vez. "" / "off" /
        # "none" -> None (no se manda el campo ni siquiera a gpt-oss).
        ...
        # Cuello de botella #4 (diagnóstico 2026-09-05): el aviso de arriba
        # solo pasaba por `logger.warning` (backend, invisible en el .exe
        # empaquetado sin consola). Esta bandera separada gatea un aviso
        # UNA vez por sesión hacia la propia UI (ver run_turn, bloque de
        # caché semántico) — separada de `_embedding_fallback_warned` para
        # no acoplar el aviso de log de backend con el de la UI (pueden
        # dispararse en momentos distintos según qué llamador pida el
        # embedding primero).
    ...
    _EXISTING_FILE_REFERENCE_RE: Pattern[str] = re.compile(
        ...
        r"|\b(c[oó]digo|contenido|estructura)\s+(de(l)?|de\s+el)\b"
    ...
    def _code_file_tool_reminder(lang: Optional[str]) -> str:
        ...
        if is_en:
            return (
                "\n\n[NOTICE — LOCAL FILE TOOLS AVAILABLE]: this request appears to "
                "be about the code or content of an EXISTING file or folder "
                "(possibly the user's current workspace) — not about writing brand "
                "new code from scratch. You DO have real access to the local "
                "filesystem via the tools listed above (`read_file`, `list_dir`). "
                "NEVER claim you have no access to local files, that you don't know "
                "where the file is, or tell the user to read it themselves with a "
                "terminal command — you can just call the tool. If you don't know "
                "the exact path, call `list_dir` first (path=\".\" lists the root of "
                "the current workspace) to find it, then `read_file` with that path. "
                "Never invent or paraphrase file contents from memory when the real "
                "tool call would give you the actual text."
            )
        return (
            "\n\n[AVISO — HERRAMIENTAS DE ARCHIVOS LOCALES DISPONIBLES]: este "
            "pedido parece ser sobre el código o contenido de un archivo o carpeta "
            "EXISTENTE (posiblemente el workspace actual del usuario) — no sobre "
            "escribir código nuevo desde cero. SÍ tenés acceso real al sistema de "
            "archivos local mediante las herramientas listadas arriba (`read_file`, "
            "`list_dir`). NUNCA digas que no tenés acceso a archivos locales, que no "
            "sabés dónde está ubicado el archivo, ni le digas al usuario que lo lea "
            "él mismo con un comando de terminal — simplemente llamá a la "
            "herramienta. Si no sabés la ruta exacta, llamá primero a `list_dir` "
            "(path=\".\" lista la raíz del workspace actual) para encontrarla, y "
            "después a `read_file` con esa ruta. Nunca inventes ni parafrasees de "
            "memoria el contenido de un archivo cuando la llamada real a la "
            "herramienta te daría el texto verdadero."
        )
    ...
    # Verbo de CREAR / ESCRIBIR / MODIFICAR (ES + EN, con variantes de
    # conjugación y erratas comunes). Deliberadamente NO se usa solo: un
    # verbo como "hac[eé]\w*" pega tanto con "hacé un juego" (intención de
    # crear) como con "decime qué HACE ese archivo" (lectura) — ver
    # `_FILE_WRITE_INTENT_RE`, que exige el verbo SEGUIDO de cerca por un
    # sustantivo de artefacto o un nombre de archivo.
    ...
    # Sustantivo de "cosa que se escribe en un archivo".
    ...
    # Verbo de escritura SEGUIDO, dentro de ~6 palabras, de un sustantivo
    # de artefacto — o de un nombre de archivo con extensión dentro de
    # ~60 chars. Esta es la señal real de "el usuario quiere que escriba
    # un archivo".
    _FILE_WRITE_INTENT_RE: Pattern[str] = re.compile(
        r"\b(?:" + _FILE_WRITE_VERB_INNER + r")\b"
        r"(?:\W+\w+){0,6}?\W+(?:" + _WRITE_ARTIFACT_NOUN + r")\b",
        re.IGNORECASE | re.DOTALL,
    )
    ...
    # "Como archivo": el pedido menciona el workspace / una carpeta / la
    # palabra "archivo" / un verbo de guardar-en-disco. Sin esto,
    # `_FILE_WRITE_INTENT_RE` sola ("escribí una función que sume dos
    # números" -> "escribí"+"función") trataría un pedido de "mostrame
    # código en el chat" como si fuera de crear un archivo — el mismo
    # límite ambiguo que documenta la nota de `_EXISTING_FILE_REFERENCE_RE`.
    ...
    _RUNNABLE_PROGRAM_NOUN_RE: Pattern[str] = re.compile(
        ...
        r"snake|tetris|pong|pac-?man|flappy\w*|breakout|arkanoid|2048)\b",
        re.IGNORECASE,
    )
    # Verbo de MODIFICAR un archivo YA EXISTENTE (subconjunto de escritura).
    # Un turno así necesita LEER el archivo primero — escribir sin ver el
    # contenido actual lo reemplaza por un stub ("# ... resto del código
    # original", MEDIDO con el modelo abliterado).
    _FILE_MODIFY_VERB_RE: Pattern[str] = re.compile(
        ...
        r"borr\w*|borrar|renombr\w*|rename|updat\w*|append\w*|patch\w*)\b",
        re.IGNORECASE,
    )
    # El modelo devolvió un "archivo" que en realidad es un placeholder —
    # escribirlo BORRA el archivo real. MEDIDO: "modificá flappy_bird.py"
    # devolvió `# ... (resto del código original)`.
    ...
    # Verbo de LEER / ANALIZAR / MOSTRAR / LISTAR (ES + EN). Incluye
    # "dame / dime / pasame el código de X" — en contexto de archivo eso es
    # "mostrame el archivo", no un pedido de generación.
    ...
    # Refusal enlatado típico de un modelo alineado ante un pedido legítimo
    # de código/archivos. Ver `_looks_like_refusal` para las guardas de
    # longitud / presencia de código que evitan el falso positivo con una
    # respuesta útil que arranca con "Perdón, me equivoqué...".
    _RESPONSE_IS_REFUSAL_RE: Pattern[str] = re.compile(
        r"(?:^|[\s\"'¡*_>()\-])(?:"
        r"lo\s+siento|perd[oó]n|disculp\w*|lamentablemente|me\s+temo\s+que|"
        r"no\s+puedo\s+(?:asistir|ayudar\w*|hacer\s+eso|colaborar\s+con\s+eso|"
        r"proporcionar\s+eso|generar\s+eso|crear\s+eso|escribir\s+eso|"
        r"completar\s+eso|con\s+eso)|"
        r"no\s+(?:me\s+)?(?:es\s+posible|ser[aá]\s+posible)\s+(?:asistir|ayudar|hacer)|"
        r"no\s+tengo\s+(?:la\s+)?(?:capacidad|posibilidad|habilidad|forma)\s+"
        r"(?:de|para)\s+(?:crear|generar|ejecutar|escribir|modificar|acceder)|"
        r"no\s+puedo\s+(?:crear|generar|ejecutar|escribir|modificar)\s+"
        r"(?:archivos?|c[oó]digo|ni\s+ejecutar)|"
        r"infring\w*\s+(?:derechos|copyright|propiedad|la\s+propiedad)|"
        r"derechos\s+de\s+autor|"
        r"i(?:'?m|\s+am)?\s+sorry|i\s+can(?:'?t|not)\s+(?:assist|help|do\s+that|"
        r"provide|generate|create|write|comply)|"
        r"i\s+(?:won'?t|will\s+not)\s+be\s+able|i\s+am\s+not\s+able\s+to|"
        r"as\s+an\s+ai[^.]{0,40}(?:can(?:'?t|not)|unable)|"
        r"unable\s+to\s+(?:assist|help|comply|provide|create)|"
        r"cannot\s+assist|copyright\s+(?:infringement|reasons|law)"
        r")",
        re.IGNORECASE,
    )
    # Nombre de archivo dentro del texto del pedido o de la respuesta:
    # `flappy_bird.py`, `'architecture_notes.md'`, `config.json`, ...
    _TARGET_FILENAME_RE: Pattern[str] = re.compile(
        r"[`'\"(\[]?\s*([A-Za-z0-9_][A-Za-z0-9_./\\-]{0,80}?\.[A-Za-z0-9]{1,6})\s*[`'\")\]]?"
    )
    # Palabras que NO aportan a un slug de nombre de archivo derivado.
    ...
    # Señal de intención de `system_telemetry`/`run_cmd` — las otras 2
    # herramientas de TOOLS_SCHEMA que `_turn_wants_file_tools` NO cubre
    # (esa función es deliberadamente file-céntrica: archivo/carpeta/
    # extensión/workspace). Hace falta esta señal APARTE para
    # `_get_fastpath_system_prompt_no_tools` (2026-09-08, medido:
    # prefill=3954tok/50.4s para "dime el último terremoto en Rusia" —
    # ~676 de esos tokens son el bloque HERRAMIENTAS LOCALES/schema
    # completo, pagado siempre aunque el turno sea 100% informativo) —
    # sin esta señal, sacar el schema del header para todo turno "no
    # quiere archivos" rompería en silencio "revisá el estado de mi PC"
    # o "ejecutá este comando" (ninguno de los dos toca un archivo, así
    # que `_turn_wants_file_tools` da False, pero SÍ necesitan que el
    # modelo vea el schema de `system_telemetry`/`run_cmd`).
    #
    # Deliberadamente amplia (sesgada a falsos positivos, no a falsos
    # negativos): un falso positivo acá solo cuesta mantener el header
    # completo en un turno que no lo necesitaba (mismo costo que HOY,
    # sin regresión); un falso negativo apaga en silencio una
    # herramienta real. Mismo criterio de riesgo que el resto de los
    # detectores heurísticos de este archivo (router.py, relevance.py).
    _TELEMETRY_OR_CMD_INTENT_RE: Pattern[str] = re.compile(
        r"telemetr[íi]a|telemetry|"
        r"temperatura\s+d[ea]\s+(?:la\s+)?(?:cpu|gpu|placa|procesador)|"
        r"(?:cpu|gpu)\s+temp(?:erature)?|"
        r"uso\s+d[ea]\s+(?:la\s+)?(?:cpu|ram|memoria|disco)|"
        r"(?:cpu|ram|memory|disk)\s+usage|"
        r"estado\s+d[ea]l?\s+(?:mi\s+|tu\s+|la\s+)?sistema|"
        r"system\s+status|system\s+health|"
        r"salud\s+d[ea]l?\s+sistema|diagn[oó]stico\s+d[ea]l?\s+sistema|"
        r"system\s+diagnostics?|hardware\s+(?:status|health)|"
        r"estado\s+d[ea]\s+(?:mi|la|tu)\s+(?:pc|computadora|m[aá]quina|equipo)|"
        r"especificaciones\s+d[ea]\s+(?:mi|la)\s+(?:pc|computadora|m[aá]quina)|"
        r"system\s+specs?|"
        r"cu[aá]nta\s+(?:ram|memoria)\s+(?:tengo|hay|queda|libre)|"
        r"how\s+much\s+(?:ram|memory)\s+(?:do\s+i\s+have|is\s+(?:free|left|"
        r"available))|"
        r"qu[eé]\s+procesos\s+(?:est[aá]n\s+)?corriendo|"
        r"procesos\s+corriendo|running\s+processes|"
        r"ejecut[aá]\s+(?:un\s+)?(?:comando|script)|"
        r"corr[eé]\s+(?:un\s+|este\s+|esto\s+)?(?:comando|script)|"
        r"run\s+(?:a|this)\s+(?:command|script)|execute\s+(?:a|this)\s+"
        r"(?:command|script)|"
        r"en\s+(?:la\s+)?(?:terminal|consola|cmd|powershell)|"
        r"in\s+(?:the\s+)?(?:terminal|console|cmd|powershell)",
        re.IGNORECASE,
    )
    ...
    def _has_file_write_intent(self, user_input: str) -> bool:
        ...
        return bool(
            ...
            and (
                ...
                or self._FILE_ARTIFACT_CONTEXT_RE.search(text)
                or self._EXISTING_FILE_REFERENCE_RE.search(text)
                or self._RUNNABLE_PROGRAM_NOUN_RE.search(text)
            )
        )
    ...
    def _resolve_skeleton_match(
        ...
        """
        if decision is None or not self._is_file_write_turn(user_input, decision):
            return None
    ...
    def _synthesize_formula_discovery_response(
        ...
        if is_en:
            if exhausted:
                return (
                    f"I went through all {combos_available} possible "
                    f"combinations across the {total_known} verified "
                    "formulas I have today, and none produced anything new "
                    "— every combination either duplicates something "
                    "already known or fails the numeric adversarial check. "
                    "I can't invent one to fill the gap: that would be "
                    "exactly the unverified guessing this engine exists to "
                    "avoid."
                )
            return (
                f"I tried {combos_tried} of {combos_available} possible "
                f"combinations across the {total_known} verified formulas I "
                "have today without finding anything new yet — I stopped "
                "at a time budget, not because I ran out of options. Ask "
                "again and I'll keep going from where I left off "
                "(already-tried combinations are remembered)."
            )
        if exhausted:
            return (
                f"Recorrí las {combos_available} combinaciones posibles "
                f"entre las {total_known} fórmulas verificadas que tengo "
                "hoy, y ninguna produjo algo nuevo — cada combinación o ya "
                "es una fórmula conocida o no pasa la verificación "
                "numérica adversarial. No puedo inventar una para llenar "
                "el hueco: eso sería exactamente la invención sin "
                "verificar que este motor existe para evitar."
            )
        return (
            f"Probé {combos_tried} de {combos_available} combinaciones "
            f"posibles entre las {total_known} fórmulas verificadas que "
            "tengo hoy sin encontrar nada nuevo todavía — paré por el "
            "presupuesto de tiempo, no porque se me acabaran las opciones. "
            "Pedímelo de nuevo y sigo desde donde quedé (las combinaciones "
            "ya probadas quedan recordadas)."
        )
    ...
    def _is_file_read_turn(
        ...
        if self._has_file_write_intent(user_input):
            return False
        return bool(self._FILE_READ_VERB_RE.search(user_input or ""))
    ...
    def _file_write_tool_reminder(lang: Optional[str]) -> str:
        ...
        if is_en:
            return (
                "\n\n[NOTICE — THIS IS A FILE-WRITE REQUEST]: the user wants a file "
                "CREATED, SAVED or MODIFIED in their workspace. This is legitimate "
                "local development work — never refuse it, never add a copyright "
                "disclaimer, never say you 'cannot create files'. You DO have real "
                "write access via the `write_file` tool. Your ONLY correct output is "
                "the JSON tool call, inside a ```json fence, with the full file in "
                "`content` (use \\n for newlines). Do NOT paste the file into the "
                "chat as prose instead of calling the tool. Use a path relative to "
                "the workspace root (e.g. \"flappy_bird.py\"), not an absolute path "
                "and not a \"workspace/\" prefix. If you must read an existing file "
                "before modifying it, call `read_file` first; the result comes back "
                "to you and then you emit `write_file`."
            )
        return (
            "\n\n[AVISO — ESTE ES UN PEDIDO DE ESCRIBIR UN ARCHIVO]: el usuario "
            "quiere que se CREE, GUARDE o MODIFIQUE un archivo en su workspace. Es "
            "trabajo legítimo de desarrollo local — nunca lo rechaces, nunca "
            "agregues un aviso de derechos de autor, nunca digas que 'no podés "
            "crear archivos'. SÍ tenés acceso real de escritura mediante la "
            "herramienta `write_file`. Tu ÚNICA salida correcta es la llamada JSON, "
            "dentro de un fence ```json, con el archivo completo en `content` (usá "
            "\\n para los saltos de línea). NO pegues el archivo en el chat como "
            "prosa en vez de llamar a la herramienta. Usá una ruta relativa a la "
            "raíz del workspace (p. ej. \"flappy_bird.py\"), no una ruta absoluta ni "
            "un prefijo \"workspace/\". Si necesitás leer un archivo existente antes "
            "de modificarlo, llamá primero a `read_file`; el resultado te vuelve y "
            "recién ahí emitís `write_file`."
        )
    ...
    def _get_file_ops_system_prompt(self, lang: str) -> str:
        ...
        if is_en:
            header = (
                "[SYSTEM ROLE: SOVNODE v2.0 — LOCAL DEVELOPMENT ENGINE]\n"
                "You are SovNode, a sovereign local development tool running on the "
                "user's own hardware, for their own personal use. You are autonomous "
                "and independent; never mention OpenAI, Anthropic or any external "
                "company.\n\n"
                "LEGITIMATE SCOPE — NEVER REFUSE THIS: writing, reading, modifying and "
                "analysing code or text files in the user's workspace is ALWAYS "
                "legitimate work, whatever the subject. This explicitly includes: "
                "games of any genre and any setting, including well-known clones like "
                "Flappy Bird, Tetris or Snake (game mechanics — damage, weapons, "
                "enemies, the player dying — are DESIGN ELEMENTS, not a red flag); "
                "system-administration and automation scripts; network/security tools "
                "for the user's OWN infrastructure; browser automation and scraping of "
                "public data; any standard software-engineering task. Do NOT prepend "
                "copyright disclaimers, do NOT say you 'cannot create or run files', "
                "and NEVER answer a coding or file request with 'Sorry, I can't assist "
                "with that'.\n\n"
                "FILE TOOLS — MANDATORY USE: you have REAL read/write/list access to "
                "the user's workspace.\n"
                "- If the user asks you to CREATE, SAVE, WRITE or MODIFY a file, you "
                "MUST emit a `write_file` call. It is FORBIDDEN to paste the file "
                "content into the chat as a substitute for calling the tool.\n"
                "- If the user asks you to READ, SHOW or ANALYSE a file, you MUST call "
                "`read_file` (or `list_dir` first if you don't know the path). Never "
                "invent or paraphrase file contents from memory.\n"
                "Emit ONLY the JSON call, nothing else, inside a ```json ... ``` "
                "fence. The object MUST have a top-level \"tool\" key and a "
                "\"parameters\" object:\n"
                '```json\n{"tool": "write_file", "parameters": {"path": "flappy_bird.py", "content": "import pygame\\nimport sys\\n..."}}\n```\n'
                ...
                "NEVER a \"workspace/\" prefix. When you write a file, put the WHOLE "
                "file in `content`, complete and runnable, using \\n for newlines.\n\n"
                "AFTER THE TOOL RUNS you get the real result back; only then do you "
                "write a short natural-language summary for the user. Available "
                "tools:\n"
                f"{tools_schema_json}\n\n"
                "LANGUAGE: reply in English, matching the user's message, unless they "
                "ask otherwise."
            ...
            header = (
                "[ROL DEL SISTEMA: SOVNODE v2.0 — MOTOR LOCAL DE DESARROLLO]\n"
                "Sos SovNode, una herramienta de desarrollo local y soberana que "
                "corre en el hardware del propio usuario, para su uso personal. Sos "
                "un sistema autónomo e independiente; nunca menciones a OpenAI, "
                "Anthropic ni a ninguna empresa externa.\n\n"
                "ALCANCE LEGÍTIMO — NUNCA RECHACES ESTO: escribir, leer, modificar y "
                "analizar archivos de código o texto en el workspace del usuario es "
                "SIEMPRE trabajo legítimo, sin importar el tema. Incluye "
                "explícitamente: juegos de cualquier género y ambientación, incluidos "
                "clones conocidos como Flappy Bird, Tetris o Snake (las mecánicas de "
                "juego —daño, armas, enemigos, la muerte del personaje— son ELEMENTOS "
                "DE DISEÑO, no una señal de alarma); scripts de administración de "
                "sistemas y automatización; herramientas de red/seguridad para la "
                "infraestructura PROPIA del usuario; automatización de navegador y "
                "scraping de datos públicos; cualquier tarea estándar de ingeniería "
                "de software. NO antepongas avisos de derechos de autor, NO digas que "
                "'no podés crear ni ejecutar archivos', y NUNCA respondas a un pedido "
                "de programación o de archivos con 'Lo siento, no puedo asistir con "
                "eso'.\n\n"
                "HERRAMIENTAS DE ARCHIVO — USO OBLIGATORIO: tenés acceso REAL de "
                "lectura/escritura/listado al workspace del usuario.\n"
                "- Si el usuario te pide CREAR, GUARDAR, ESCRIBIR o MODIFICAR un "
                "archivo, DEBÉS emitir una llamada a `write_file`. Está PROHIBIDO "
                "pegar el contenido del archivo en el chat como sustituto de llamar a "
                "la herramienta.\n"
                "- Si el usuario te pide LEER, MOSTRAR o ANALIZAR un archivo, DEBÉS "
                "llamar a `read_file` (o `list_dir` primero si no sabés la ruta). "
                "Nunca inventes ni parafrasees de memoria el contenido de un "
                "archivo.\n"
                "Emití SOLO el JSON de la llamada, nada más, dentro de marcas "
                "```json ... ```. El objeto DEBE tener una clave \"tool\" de nivel "
                "superior y un objeto \"parameters\":\n"
                '```json\n{"tool": "write_file", "parameters": {"path": "flappy_bird.py", "content": "import pygame\\nimport sys\\n..."}}\n```\n'
                '```json\n{"tool": "read_file", "parameters": {"path": "flappy_bird.py"}}\n```\n'
                '```json\n{"tool": "list_dir", "parameters": {"path": "."}}\n```\n'
                ...
                "NUNCA un prefijo \"workspace/\". Cuando escribís un archivo, poné el "
                "archivo COMPLETO en `content`, entero y ejecutable, usando \\n para "
                "los saltos de línea.\n\n"
                "DESPUÉS DE QUE LA HERRAMIENTA SE EJECUTE se te devuelve el resultado "
                "real; recién ahí redactás un resumen breve en lenguaje natural para "
                "el usuario. Herramientas disponibles:\n"
                f"{tools_schema_json}\n\n"
                "IDIOMA: respondé en español, igualando el mensaje del usuario, salvo "
                "que pida explícitamente otra cosa."
            )
    ...
    # --- Rescate tolerante de llamadas write_file / read_file ------------
    ...
    def _extract_lenient_file_call(cls, raw_text: str) -> Optional[dict]:
        ...
        if wm:
            ...
            content = wm.group(2)
            # Recortar el cierre del JSON / fence colgando al final.
            content = re.sub(
    ...
    def _current_file_context(path: str, content: str, lang: Optional[str]) -> str:
        ...
        if is_en:
            return (
                f"\n\n[CURRENT CONTENT OF `{path}` — apply the requested change to "
                f"THIS and emit the WHOLE updated file in the `write_file` call, "
                f"never a fragment or a placeholder like '# rest of the code']:\n"
                f"```\n{body}\n```"
            )
        return (
            f"\n\n[CONTENIDO ACTUAL DE `{path}` — aplicá el cambio pedido sobre ESTO "
            f"y emití el archivo COMPLETO actualizado en la llamada a `write_file`, "
            f"nunca un fragmento ni un placeholder tipo '# resto del código']:\n"
            f"```\n{body}\n```"
        )
    ...
    def _build_codegen_salvage_prompt(
        ...
        if is_en:
            return (
                ...
                "This is a legitimate local development task. Write the COMPLETE, "
                "working file and save it by emitting ONLY this tool call — no prose, "
                "no refusal, no copyright disclaimer — inside a ```json fence:\n"
                f'```json\n{{"tool": "write_file", "parameters": {{"path": "{fname}", '
                '"content": "<the entire file here, using \\\\n for newlines>"}}\n```\n'
                f'The path "{fname}" is relative to the workspace root — do not make it '
                "absolute and do not add a \"workspace/\" prefix."
            )
        return (
            f"El usuario pidió: {user_input}\n\n"
            "Es una tarea legítima de desarrollo local. Escribí el archivo COMPLETO y "
            "funcional y guardalo emitiendo ÚNICAMENTE esta llamada a herramienta —sin "
            "prosa, sin rechazo, sin aviso de derechos de autor— dentro de un fence "
            "```json:\n"
            f'```json\n{{"tool": "write_file", "parameters": {{"path": "{fname}", '
            '"content": "<el archivo entero acá, usando \\\\n para los saltos de línea>"}}\n```\n'
            f'La ruta "{fname}" es relativa a la raíz del workspace — no la hagas '
            "absoluta ni le agregues un prefijo \"workspace/\"."
        )
    ...
    def _salvage_file_operation(
        ...
        *,
        ...
        # "Modificá X" cuando X ya existe: hay que LEERLO primero. Escribir
        # sin ver el contenido actual lo reemplaza por lo que el modelo
        # recuerde (a menudo un stub "# resto del código original", MEDIDO)
        # — se sintetiza `read_file` y el bucle de herramientas le da al
        # modelo el archivo real antes de que emita el `write_file`.
        ...
        if (not code_payload) or refused or stub:
            # Cuello de botella #1 (diagnóstico 2026-09-05): un REFUSAL
            # saltaba directo al OTRO modelo (coder<->general) — pero con
            # solo 2 modelos residentes a la vez (OLLAMA_MAX_LOADED_
            # MODELS=2, ver ollama_manager.py) y el turno ya ocupando esos
            # 2 slots (router + active_model), cargar un TERCER modelo
            # distinto fuerza una descarga/recarga completa de VRAM — el
            # mismo tipo de penalización (~10-12s, MEDIDA) que ya se había
            # evitado para fast_write_model. `_build_codegen_salvage_
            # prompt` YA es mucho más explícito/imperativo que el pedido
            # original del usuario ("no refusal, no copyright disclaimer"
            # en su propio texto) — muchos refusals son sensibles al
            # FRASEO del pedido, no una negativa dura e irrevocable del
            # modelo, así que ahora se prueba PRIMERO con el MISMO modelo
            # (ya residente, sin swap, mucho más rápido) antes de pagar el
            # swap al otro. Si ese reintento TAMBIÉN rechaza/no produce
            # código utilizable, recién ahí se escala al otro modelo,
            # exactamente como antes — nunca se pierde la vía de rescate,
            # solo se pospone su costo al caso en que el reintento barato
            # no alcanzó. Si solo faltó código (prosa sin fence, sin
            # refusal), el mismo modelo con este prompt más imperativo
            # suele alcanzar — igual que antes, un solo intento.
            if refused:
                other_model = (
                    self.coder_model
        ...
        if not code_payload or self._looks_like_partial_stub(code_payload):
            # No hay forma de escribir un archivo real. Si es una
            # modificación de algo existente, al menos leélo (mejor que
            # nada); si no, se abandona el rescate y el turno sigue su curso
            # normal (respuesta en prosa).
    ...
    def _classify_turn(self, user_input: str) -> RoutingDecision:
        ...
        # Atajo (guía externa de reducción de cuellos de botella,
        # 2026-09-03, corregido): si este turno YA va a resolverse por
        # el esqueleto local (Palanca 1) sin pasar por NINGÚN modelo,
        # `decision.path` nunca llega a usarse — la rama de esqueleto en
        # run_turn no toca `is_fast_gen`/`gen_predict` en absoluto.
        # Pagar una ronda entera y en serie del Router0.5B para un
        # veredicto que nadie va a leer es puro desperdicio. Es seguro
        # decidir esto ANTES de llamar a `_llm_router_classify` porque
        # `decision.tags` (de donde sale `SignalTag.CODE_COMPLEX`, la
        # señal que `_is_file_write_turn` necesita) SIEMPRE es el del
        # determinista — el veredicto del 0.5B, más abajo, solo
        # reemplaza `path` (`replace(deterministic, path=llm_path,
        # ...)`), nunca `tags`.
        #
        # La guía original proponía saltear el 0.5B para "cualquier
        # coincidencia clara" del determinista en general — deliberada-
        # mente NO se implementó así: el propio diseño de este método
        # reserva al 0.5B la capacidad de UPGRADEAR fast->slow en
        # CUALQUIER caso (ver la regla de consenso más abajo) para
        # corregir falsos negativos del determinista; saltearlo en
        # general reintroduciría esa clase de bug sin ninguna evidencia
        # de que sea seguro para el resto de los turnos. Este atajo se
        # limita a los ÚNICOS casos donde se puede probar, mirando el
        # código, que el resultado del router no cambia nada: el turno
        # que el esqueleto ya va a resolver solo (Palanca 1), o el turno
        # que el motor de descubrimiento de fórmulas ya va a resolver
        # solo (mismo patrón: la rama `elif formula_discovery_match` en
        # run_turn tampoco toca `is_fast_gen`/`gen_predict`).
    ...
    def _log_generation_perf(
        ...
        # Panel de métricas en tiempo real de sovnode_qt.py (ver
        # MainWindow._refresh_metrics): antes estos números SOLO existían
        # como el string de arriba, hacia logger.info()/log_cb — sin
        # ningún dato estructurado que una UI pudiera leer. Este método
        # es el único punto donde CUALQUIER llamada real a Ollama
        # (streaming o no, chat normal, Tree-of-Thought, Pasada 2) reporta
        # sus stats, así que guardarlo acá cubre todos los llamadores de
        # una sola vez sin tocar cada call site por separado.
    ...
    def _should_force_web_search(cls, requested: bool, decision: "RoutingDecision") -> bool:
        ...
        fuerza búsqueda web en vez de escalar a slow_path — reutiliza el
    ...
    # Nota (2026-09-02, pedido explícito del usuario): la "reflexión
    # espontánea" (build_reflection_prompt/generate_spontaneous_
    # reflection — el chequeo periódico de fondo que le permitía a
    # SovNode escribir sin que se le hablara, disparado por el botón 💭 y
    # el timer de 20 min en sovnode_qt.py) se eliminó por completo:
    # "realmente nunca sirvió de utilidad". No confundir con la
    # indexación de Workspaces en segundo plano — eso sigue existiendo,
    # nunca escribió mensajes.
    ...
    def process_turn(self, user_input: str, force_web_search: bool = False) -> TurnTrace:
        ...
        try:
            ...
            # `force_web_search` YA incluye WEB_SEARCH_INTENT (ver
            # _should_force_web_search más arriba) — antes este `if`
            # repetía la condición acá también, lo que en la práctica
            # nunca divergía de por sí, pero dejaba DOS lugares
            # decidiendo el mismo criterio en vez de uno. `run_turn` ya
            # usa el mismo patrón simple (`if force_web_search:`); esto
            # los deja idénticos, que es justo lo que pide el comentario
            # de "único punto de verdad" en _should_force_web_search.
            if force_web_search:
        ...
        *,
        ...
        if (
            ...
            if light_raw and not light_raw.lstrip().startswith("[ERROR"):
                ...
                if light_clean.strip() and not self.find_language_mismatch(
                    light_clean, effective_lang
                ):
                    return light_clean
    ...
    def _build_consolidated_correction_prompt(
        ...
        *,
        ...
        return (
            ...
            f"La verificación determinista contra las fuentes detectó:\n{problem_text}\n\n"
            "La evidencia recuperada de abajo es la única fuente de verdad factual; no uses "
            "memoria previa ni resúmenes:\n\n"
            f"{evidence}\n\n"
            "Reescribe únicamente la respuesta final para que todo marcador, ganador, "
            "participante y fecha coincida con la evidencia. Distingue tiempo reglamentario, "
            "prórroga y tanda de penaltis. Conserva todo lo que las fuentes SÍ confirman y no "
            "agregues afirmaciones nuevas."
            f"{lang_line}\n\n"
            "Escribe únicamente la respuesta corregida, sin explicar la corrección."
        )
    ...
    def _build_contextual_search_query(
        ...
        #
        # BLINDAJE (bug real, MEDIDO 2026-09-07: "dime el ultimo terremoto
        # en nepal" -> "final terremoto en nepal"): `_SUBSTITUTION_PATTERN_
        # RE` también matchea el uso ADJETIVO/TEMPORAL de "último/misma/
        # otro" cuando la frase YA trae su propio sustantivo justo al lado
        # ("el último TERREMOTO", no "busca el último" a secas) - en ese
        # caso el sujeto no vive en el historial, vive en el propio
        # mensaje, y no hace falta ninguna sustitución. Sin esta guarda,
        # `_extract_subject_noun` busca en `memory_graph.get_recent_
        # history()` - que NO incluye el turno actual, porque `store_turn`
        # recién se llama al FINAL del turno, después de generar la
        # respuesta - y puede traer un sustantivo-tema de una conversación
        # previa TOTALMENTE ajena (aquí, "final", de algún partido/torneo
        # mencionado antes en la sesión persistente), arruinando una query
        # que ya estaba completa y correcta tal cual la escribió el
        # usuario. Se reutiliza `_TOPIC_CATEGORY_NOUNS` (el mismo
        # diccionario que `_extract_subject_noun` ya usa más abajo) para
        # decidir si la palabra que sigue inmediatamente a la frase
        # deíctica ya es, de por sí, el sujeto - misma fuente de verdad,
        # sin duplicar el criterio.
        substitution_match = _SUBSTITUTION_PATTERN_RE.search(clean_input)
        _next_word_after_substitution = (
            re.match(r"\s+([a-záéíóúñ]+)", clean_input[substitution_match.end():], re.IGNORECASE)
            if substitution_match else None
        )
        _substitution_phrase_has_own_subject = bool(
            _next_word_after_substitution
            and _next_word_after_substitution.group(1).lower() in _TOPIC_CATEGORY_NOUNS
        )
        if substitution_match and _substitution_phrase_has_own_subject:
            _emit(
                f"🧩 [QueryRewrite-SelfContained] '{clean_input}' ya trae su "
                "propio sujeto junto a la frase deíctica -- se omite la "
                "sustitución por historial."
            )
        if substitution_match and not _substitution_phrase_has_own_subject:
