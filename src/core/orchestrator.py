...
"2. Si en tu bloque <thought> determinas que debes invocar una herramienta..."
        ...
        try:
            ...
            if clean_response and not breaker_fired:
                ...
                if (factual_hits or lang_hit) and not cancelled():
                    ...
                    # slow_path conserva su header por defecto.
                    ...
                    if correction_prompt:
                        ...
                        corrected = self._call_llm(
                            ...
                            num_predict_override=corr_num_predict,
                            log_cb=log_cb, perf_label="Verify",
                        )
                        if corrected and not corrected.lstrip().startswith("[ERROR"):
                            ...
                            clean_response = self._strip_correction_prompt_echo(
                                corrected_clean or corrected
                            )
                            # Mismo riesgo de eco del header congelado que la
                            # generación principal - ver `_strip_system_prompt_echo`.
    ...
    def _get_fastpath_system_prompt(self, lang: str) -> str:
        ...
        if is_en:
            header = (
                ...
                '```json\n{"tool": "system_telemetry", "parameters": {}}\n```\n'
                '```json\n{"tool": "write_file", "parameters": {"path": "workspace/system_health.py", "content": "import psutil\\n..."}}\n```\n'
    ...
    def _select_model_for_decision(self, decision: RoutingDecision) -> str:
        ...
        respuesta (`self.model`, qwen2.5:7b por defecto — ver
