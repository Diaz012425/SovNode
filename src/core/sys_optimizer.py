...
ES: Recolecta telemetría de hardware (RAM, disco, GPU, procesos) para sugerir optimizaciones.
EN: Collects hardware telemetry (RAM, disk, GPU, processes) to suggest optimizations.
"""
...
# ES: Evita que cada subprocess.run() (PowerShell/nvidia-smi) abra su propia
# ventana de consola visible en Windows cuando la app empaquetada no tiene
# consola propia a la que adjuntarse.
# EN: Prevents each subprocess.run() (PowerShell/nvidia-smi) from opening its
# own visible console window on Windows when the packaged app has no console of its own.
CREATE_NO_WINDOW = 0x08000000

# ES: TTL de caché para no repetir los 3 subprocess.run() (2x PowerShell + nvidia-smi)
# en llamadas seguidas que no necesitan datos más frescos.
# EN: Cache TTL so the 3 subprocess.run() calls (2x PowerShell + nvidia-smi) aren't
# repeated on back-to-back calls that don't need fresher data.
...
def get_system_telemetry() -> Dict[str, Any]:
    """ES: Recopila información técnica y consumo actual del sistema (con caché TTL, ver TELEMETRY_CACHE_TTL_SECONDS).
    EN: Collects current system technical info and resource usage (TTL-cached, see TELEMETRY_CACHE_TTL_SECONDS)."""
...
def _compute_system_telemetry() -> Dict[str, Any]:
    """ES: Ejecuta las consultas reales de SO, RAM, disco, procesos y GPU (sin caché).
    EN: Runs the actual OS, RAM, disk, process, and GPU queries (uncached)."""
    telemetry: Dict[str, Any] = {}

    # ES: Información básica del sistema operativo y CPU.
    # EN: Basic OS and CPU information.
    telemetry["os"] = f"{platform.system()} {platform.release()} ({platform.architecture()[0]})"
    telemetry["cpu_count_logical"] = os.cpu_count() or 0

    # ES: Memoria RAM mediante PowerShell WMI (sin depender de psutil).
    # EN: RAM via PowerShell WMI (no psutil dependency).
    try:
        ps_script = (
    ...
    # ES: Espacio en disco principal. / EN: Main disk space.
    try:
    ...
    # ES: Procesos que más RAM consumen. / EN: Processes consuming the most RAM.
    try:
        ps_top = (
    ...
    # ES: Verificación de GPU (nvidia-smi si existe). / EN: GPU check (nvidia-smi if present).
    try:
...
def build_optimization_prompt() -> str:
    """ES: Genera el reporte estructurado de hardware para que el orquestador proponga optimizaciones.
    EN: Builds the structured hardware report so the orchestrator can suggest optimizations."""
