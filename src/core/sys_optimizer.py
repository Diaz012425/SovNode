...
# BLINDAJE (bug real, reportado: cada ~5s se veían destellar 2 ventanas de
# PowerShell — más nvidia-smi — que se cerraban al instante): ninguno de
# los 3 subprocess.run() de abajo pasaba `creationflags`, así que cada
# spawn creaba su PROPIA consola nueva y visible (comportamiento por
# defecto de CreateProcess en Windows para un binario de consola cuando el
# proceso padre — SovNode empaquetado, `console=False` en el .spec — no
# tiene una consola propia a la que adjuntarse). Coincide exacto con
# TELEMETRY_CACHE_TTL_SECONDS=5.0: MainWindow.metrics_timer (sovnode_qt.py)
# llama a get_system_telemetry() cada 3s mientras el panel de terminal
# está visible; cada vez que la caché de 5s expira, se repiten los 3
# spawns. Mismo patrón (`creationflags=CREATE_NO_WINDOW`) que
# ollama_manager.py ya usa de forma consistente en sus propios
# subprocess.Popen/run — se replica acá.
