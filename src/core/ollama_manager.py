...
class OllamaProcessManager:
    ...
    # `OLLAMA_MAX_LOADED_MODELS=2` porque cada turno usa DOS modelos
    # distintos: el router (self.router_model, qwen2.5:0.5b, ~0.4GB) para
    # clasificar fast/slow, y el de respuesta (RESPONSE_MODEL, qwen2.5:7b,
    # ~4.7GB q4) para generar. Sin esto, Ollama descargaría el de 7B de la
    # VRAM en cada llamada al router y lo recargaría para generar — dos
    # cargas completas por turno. Con `=2` + `keep_alive=30m` en ambas
    # llamadas, los dos quedan residentes... SIEMPRE QUE la VRAM aguante
    # ~5.5GB juntos. En VRAM compartida/iGPU chica igual puede haber
    # thrashing: confirmar con `ollama ps` mientras se usa la app. Subir
    # esto sin medir VRAM real disponible puede degradar el throughput por
    # contención en vez de mejorarlo.
