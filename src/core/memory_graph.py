...
class MemoryGraph:
    ...
    def _init_db(self) -> None:
        with self._get_connection() as conn:
            ...
            # Carpetas activas de "Workspaces" (ver WorkspaceScanner en
            # workspace_watcher.py y la card "Workspaces" de sovnode_qt.py).
            # Antes vivían SOLO en memoria de proceso (WorkspaceScanner._roots)
            # y en la QListWidget de la UI: cerrar la app perdía por
            # completo la lista, y el usuario tenía que volver a agregar
            # cada carpeta a mano en cada arranque. `path` es la clave
            # primaria (natural — no tiene sentido vigilar la misma ruta
            # dos veces) y el `INSERT OR IGNORE` de add_workspace_folder()
            # se apoya en eso para ser idempotente sin un SELECT previo.
            cursor.execute("""
            ...
            # Estado vivo (mutable) de la síntesis de conocimiento en
            # inactividad: el axioma en sí y su verificación son
            # INMUTABLES y viven en el WAL como KnowledgeNode
            # (domain="synthetic_knowledge", ver wal.py) — esta tabla NO
            # duplica ese contenido, solo referencia su node_id
            # (`node_id_ref`, la clave primaria natural: un KnowledgeNode
            # ya es único por hash SHA-256 de dominio+axioma+verificación,
            # así que reusar ese hash como PK evita generar un id nuevo y
            # hace que re-proponer el mismo nodo sea idempotente vía
            # INSERT OR IGNORE) y guarda lo que SÍ cambia con el tiempo:
            # `confidence_score` (puede recalibrarse), `use_count`/
            # `last_reused_at` (igual que `validated_tools.use_count` —
            # se incrementa cada vez que `fetch_hybrid_context()` termina
            # usando este nodo) y `created_at` para la purga mensual de
            # nodos sintéticos que nunca se reusaron.
            cursor.execute("""
                CREATE TABLE IF NOT EXISTS synthetic_knowledge (
                    node_id_ref TEXT PRIMARY KEY,
    ...
    def add_workspace_folder(self, path: str) -> None:
        """
        Persiste una carpeta de "Workspaces" para que sobreviva a un
        reinicio de la app — antes solo vivía en `WorkspaceScanner._roots`
        (memoria de proceso) y en la `QListWidget` de la UI. `INSERT OR
        IGNORE` la vuelve idempotente: agregar la misma ruta dos veces (el
        usuario reabre el diálogo y elige la misma carpeta) no falla ni
        duplica la fila, simplemente no hace nada la segunda vez.
        """
        with self._get_connection() as conn:
            conn.execute(
                "INSERT OR IGNORE INTO workspace_folders (path, added_at) VALUES (?, ?)",
                (path, time.time()),
            )
            conn.commit()

    def remove_workspace_folder(self, path: str) -> None:
        """Contraparte de add_workspace_folder(). No lanza si `path` no
        estaba persistido (DELETE sin filas afectadas es un no-op)."""
        with self._get_connection() as conn:
            conn.execute("DELETE FROM workspace_folders WHERE path = ?", (path,))
            conn.commit()

    def list_workspace_folders(self) -> List[str]:
        """
        Rutas de "Workspaces" persistidas, en el orden en que se
        agregaron. Usado en el arranque de MainWindow para repoblar
        `WorkspaceScanner`/`workspaces_list` sin que el usuario tenga que
        volver a agregar cada carpeta a mano. No valida que la ruta siga
        existiendo en disco — igual que `WorkspaceScanner.add_root()`,
        una carpeta que ya no está (drive externo desconectado, etc.)
        simplemente no aporta archivos al próximo escaneo, sin romper
        nada.
        """
        with self._get_connection() as conn:
            cursor = conn.execute(
                "SELECT path FROM workspace_folders ORDER BY added_at ASC"
            )
            return [row["path"] for row in cursor.fetchall()]

    def add_synthetic_knowledge(self, node_id_ref: str, confidence_score: float) -> None:
        """
        Registra el estado vivo de un KnowledgeNode sintético ya
        persistido en el WAL (domain="synthetic_knowledge"). `INSERT OR
        IGNORE` por la misma razón que `add_workspace_folder`: si el
        daemon de síntesis vuelve a proponer el mismo par (N1, N2) —
        mismo axioma, mismo node_id_ref por ser un hash de contenido —
        no duplica la fila ni pisa un `confidence_score`/`use_count` que
        ya pudo haber cambiado desde entonces.
        """
        with self._get_connection() as conn:
            conn.execute(
                "INSERT OR IGNORE INTO synthetic_knowledge "
                "(node_id_ref, confidence_score, created_at, last_reused_at, use_count) "
                "VALUES (?, ?, ?, NULL, 0)",
                (node_id_ref, confidence_score, time.time()),
            )
            conn.commit()

    def remove_synthetic_knowledge(self, node_id_ref: str) -> None:
        """Contraparte de add_synthetic_knowledge(). No lanza si el nodo
        ya no estaba (DELETE sin filas afectadas es un no-op). No toca
        el WAL: el KnowledgeNode inmutable subyacente queda en el log de
        auditoría de todos modos — esto solo retira el nodo del estado
        vivo consultable (p. ej. tras la purga mensual, o si el juez lo
        marca luego como inválido)."""
        with self._get_connection() as conn:
            conn.execute("DELETE FROM synthetic_knowledge WHERE node_id_ref = ?", (node_id_ref,))
            conn.commit()

    def get_synthetic_knowledge(self, node_id_ref: str) -> Optional[Dict[str, Any]]:
        """Estado vivo actual de un nodo sintético, o None si no está
        (nunca se propuso, o ya fue purgado/retirado)."""
        with self._get_connection() as conn:
            cursor = conn.execute(
                "SELECT node_id_ref, confidence_score, created_at, last_reused_at, use_count "
                "FROM synthetic_knowledge WHERE node_id_ref = ?",
                (node_id_ref,),
            )
            row = cursor.fetchone()
            if row is None:
    ...
    def mark_synthetic_knowledge_reused(self, node_id_ref: str) -> None:
        """
        Marca un nodo sintético como reusado en un turno real — mismo
        patrón que el `UPDATE ... use_count = use_count + 1` de
        `fetch_validated_tool()`. Un nodo con `use_count` alto es
        evidencia de que la conexión sintética resultó útil en la
        práctica, no solo válida en el momento de proponerse; la purga
        mensual se apoya en esta señal para no borrar nodos que sí están
        demostrando valor aunque sean viejos.
        """
        with self._get_connection() as conn:
            conn.execute(
                "UPDATE synthetic_knowledge "
                "SET use_count = use_count + 1, last_reused_at = ? "
                "WHERE node_id_ref = ?",
                (time.time(), node_id_ref),
            )
            conn.commit()

    def purge_stale_synthetic_knowledge(self, max_age_days: float = 30.0) -> List[str]:
        """
        Purga mensual del estado vivo: retira los nodos sintéticos con
        más de `max_age_days` desde su creación que JAMÁS se reusaron
        (`use_count == 0`) — un nodo que ya demostró valor (se usó al
        menos una vez en un turno real) se conserva sin importar su
        antigüedad. Solo borra de `synthetic_knowledge` (el estado vivo
        consultable); el KnowledgeNode inmutable correspondiente sigue
        en el WAL para siempre, como registro de auditoría.

        Devuelve la lista de `node_id_ref` purgados (no solo la cantidad):
        el llamador (KnowledgeSynthesizer, al arrancar cada ciclo) la usa
        para además retirar el vector correspondiente de
        `longterm_vector_rag` — sin los ids, no podría saber CUÁLES
        vectores quedaron huérfanos.
        """
        cutoff = time.time() - (max_age_days * 86400.0)
        with self._get_connection() as conn:
            cursor = conn.execute(
                "SELECT node_id_ref FROM synthetic_knowledge WHERE use_count = 0 AND created_at < ?",
                (cutoff,),
            )
            purged_ids = [row["node_id_ref"] for row in cursor.fetchall()]
            if purged_ids:
                conn.execute(
                    "DELETE FROM synthetic_knowledge WHERE use_count = 0 AND created_at < ?",
                    (cutoff,),
                )
                conn.commit()
            return purged_ids

    def fetch_web_knowledge_since(self, since_ts: float, limit: int = 50) -> List[Dict[str, Any]]:
        """
        Entradas de `web_knowledge` persistidas después de `since_ts`.
        Usado por KnowledgeSynthesizer para el muestreo incremental de su
        pipeline de síntesis en inactividad: cada ciclo solo procesa lo
        NUEVO desde el ciclo anterior, en vez de reescanear toda la tabla
        cada vez (costo que crecería sin cota con el uso del programa).
        """
        with self._get_connection() as conn:
            cursor = conn.execute(
                "SELECT id, content, fetched_at FROM web_knowledge "
                "WHERE fetched_at > ? ORDER BY fetched_at ASC LIMIT ?",
                (since_ts, limit),
            )
            return [
                {"id": row["id"], "content": row["content"], "created_at": row["fetched_at"]}
                for row in cursor.fetchall()
            ]

    def fetch_reasoning_lessons_since(self, since_ts: float, limit: int = 50) -> List[Dict[str, Any]]:
        """Contraparte de fetch_web_knowledge_since() para `reasoning_lessons`."""
        with self._get_connection() as conn:
            cursor = conn.execute(
                "SELECT id, content, created_at FROM reasoning_lessons "
