...
class IntentRouter:
    ...
    # Deliberadamente 0.0 por el mismo motivo que WEIGHT_FACTUAL_
    # ENUMERATION -- ver la nota junto a _FORMULA_DISCOVERY_*_RE más
    # abajo: este turno se resuelve por completo en orchestrator.py
    # (`_resolve_formula_discovery_match`) sin pasar por NINGÚN modelo
    # (ni fast_path ni slow_path), así que empujar el score no cambiaría
    # nada -- `decision.path` ni siquiera se llega a usar en esa rama,
    # igual que con el esqueleto local (Palanca 1, ver skeletons.py).
    ...
    # Mejora #1 (2026-09-05, "aplica las mejoras 1 2 3 y 4"): pedido real,
    # MEDIDO (captura de la UI) que motivó esto: "me refiero, descubre una
    # ecuacion fisica que nunca se ah descubierto" -- sin esta señal, ese
    # turno caía en la rama general (sin ninguna verificación) y el LLM
    # sin censura inventaba una ecuación con apariencia de LaTeX
    # ("(d³x/dt³)² + 1 = (dx/dt · ∇V(x))") sin verificar nada, exactamente
    # lo que `formula_synthesizer.py` existe para prevenir. Distinta de
    # _MATH_PATTERN (que detecta TRABAJAR una expresión ya dada -
    # resolver/derivar/simplificar) y de _FACTUAL_ENUMERATION_PATTERN
    # (recordar N hechos YA establecidos): acá se pide construir algo
    # NUEVO, nunca antes visto. Requiere las TRES señales presentes en
    # cualquier orden (verbo de descubrimiento + marcador de novedad +
    # sustantivo de fórmula) en vez de una sola regex de orden fijo -- más
    # robusto contra la variedad real de frases ("descubre una ecuacion
    # ... que nunca se ah descubierto", "puedes descubrir una nueva
    # ecuacion usando los ladrillos...") sin arriesgar una sola regex
    # gigante fragile. Ver el uso real en orchestrator.py,
    # `_resolve_formula_discovery_match`.
    ...
    # el listado de `src/core` donde debía ir la ecuación final). El
    # lookbehind negativo excluye específicamente "en función de", la
    # frase matemática habitual -- "escribí/creá/implementá una función"
    # (código real) sigue matcheando normalmente, ninguna sigue precedida
    # de "en ".
    _CODE_COMPLEX_PATTERN: Pattern[str] = re.compile(
        ...
        r"c[oó]digo|program\w*|script\w*|scrip\w*|algoritmo\w*|algoritm\w*|(?<!en )funci[oó]n|clase|m[eé]todo|"
    ...
    def classify(self, text: str) -> RoutingDecision:
        ...
        # solo porque "match" es una palabra clave deportiva), o si es un
        # superlativo de importancia histórica sin relación real con
        # actualidad (p. ej. "el último gran hito de la humanidad") ---
        if (
