...
# ES: Modo con el que se produjo un vector. SEMANTIC = similitud coseno real
# (modelo de embeddings); HASH_FALLBACK = bag-of-words hasheado, invariante
# al orden de palabras (similitud solo indica "comparten palabras"). Cualquier
# umbral de similitud (caché semántico) debe revisar el modo antes de confiar en el número.
# EN: Mode a vector was produced in. SEMANTIC = real cosine similarity (embedding
# model); HASH_FALLBACK = hashed bag-of-words, order-invariant (similarity only
# means "shares words"). Any similarity threshold (semantic cache) must check
# the mode before trusting the number.
EMBEDDING_MODE_SEMANTIC = "semantic"
EMBEDDING_MODE_HASH_FALLBACK = "hash_fallback"
EMBEDDING_MODE_UNAVAILABLE = "unavailable"

# ES: all-MiniLM-L6-v2 — 384 dimensiones (coincide con LocalVectorRAG(vector_dim=384)),
# liviano vía ONNX (sin PyTorch), rápido en CPU.
# EN: all-MiniLM-L6-v2 — 384 dimensions (matches LocalVectorRAG(vector_dim=384)),
# lightweight via ONNX (no PyTorch), fast on CPU.
_LOCAL_EMBEDDING_MODEL_NAME = os.getenv(
    "SOVNODE_LOCAL_EMBED_MODEL", "sentence-transformers/all-MiniLM-L6-v2"
)
# ES: Caché persistente fuera de carpetas temporales, para sobrevivir a limpiezas
# de temp y reinicios de Windows sin forzar una re-descarga del modelo (~90MB).
# EN: Persistent cache outside temp folders, to survive temp cleanups and Windows
# restarts without forcing a model re-download (~90MB).
