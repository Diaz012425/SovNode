...
class WriteAheadLog:
    ...
    # BLINDAJE 2026-09-16 (auditoría de seguridad/estructura): si
    # `_do_fsync` falla FSYNC_ALERT_THRESHOLD veces seguidas, se sube de
    # `logger.error` a `logger.critical` -- una sola falla puede ser un
    # hiccup transitorio del filesystem, pero una racha sostenida es
    # señal de que el disco tiene un problema real (lleno, desmontado,
    # USB desconectado) y el usuario debería enterarse, no solo quedar
    # en el log.
    FSYNC_ALERT_THRESHOLD = 5
    ...
    def __init__(self, log_path: str | Path = "sovnode.wal") -> None:
        ...
        self._fsync_consecutive_failures = 0
        self._last_fsync_error: Optional[str] = None
    ...
    def _do_fsync(self) -> None:
        """
        ES: Vacía físicamente a disco lo pendiente. Llamada por el hilo de
        fondo, por umbral, o desde close().

        BLINDAJE 2026-09-16 (auditoría de seguridad/estructura -- bug real,
        no solo teórico): antes, un `OSError` de `os.fsync` (disco lleno,
        USB desconectado, error de I/O real) se tragaba en silencio con
        `except OSError: pass`, y `_pending_fsync` se reseteaba a 0 IGUAL,
        haya funcionado el fsync o no. Eso marcaba esas escrituras como
        "ya persistidas de forma durable" sin estarlo, sin loguear nada y
        sin ningún reintento -- si justo ahí se cortaba la luz, esos
        eventos del WAL se perdían sin ningún rastro de que el fallo había
        pasado. Fix: en fallo, NO se resetea `_pending_fsync` (así el
        próximo ciclo del hilo de fondo, o el próximo umbral de escrituras,
        reintenta el fsync de lo mismo que quedó pendiente -- el dato en sí
        ya está en el archivo vía flush(), solo falta que el SO lo baje a
        disco), se loguea el error (`logger.error`, con el motivo real de
        `OSError`), y se cuenta la racha de fallas consecutivas: a partir
        de `FSYNC_ALERT_THRESHOLD` fallas seguidas escala a
        `logger.critical` -- visible de verdad en los logs en vez de un
        `pass` mudo. `get_fsync_health()` expone este estado por si algún
        día se quiere mostrar en la UI (ver system_health.py).

        EN: Physically flushes pending writes to disk. Called by the
        background thread, on threshold, or from close(). See the
        blindaje above: an `OSError` here used to be swallowed silently
        AND `_pending_fsync` was reset regardless of success, marking
        un-synced writes as durable with zero logging and zero retry.
        Now a failure keeps `_pending_fsync` intact (so the next cycle
        retries), logs the real `OSError`, and tracks a consecutive-
        failure streak that escalates to `logger.critical` past
        `FSYNC_ALERT_THRESHOLD`.
        """
        with self._lock:
            ...
            try:
                ...
                self._fsync_consecutive_failures += 1
                self._last_fsync_error = str(exc)
                log_fn = (
                    logger.critical
                    if self._fsync_consecutive_failures >= self.FSYNC_ALERT_THRESHOLD
                    else logger.error
                )
                log_fn(
                    "[WAL] fsync() falló (%d fallas consecutivas, %d escrituras "
                    "pendientes de sincronizar a disco todavía en el archivo vía "
                    "flush(), no perdidas): %s",
                    self._fsync_consecutive_failures,
                    self._pending_fsync,
                    exc,
                )
                return
            self._fsync_consecutive_failures = 0
            self._last_fsync_error = None
            self._pending_fsync = 0

    def get_fsync_health(self) -> dict[str, Any]:
        """
        ES: Estado de salud del fsync en background, para que un llamador
        (UI, telemetría) pueda detectar una racha de fallas reales de disco
        en vez de enterarse recién cuando se pierde algo. No lanza.
        EN: Background-fsync health snapshot, so a caller (UI, telemetry)
        can detect a real streak of disk failures instead of finding out
        only after something is lost. Never raises.
        """
        with self._lock:
            return {
                "consecutive_failures": self._fsync_consecutive_failures,
                "last_error": self._last_fsync_error,
                "pending_unsynced_writes": self._pending_fsync,
            }
