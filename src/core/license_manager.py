...
# =====================================================================
# ES: Sin secreto configurado, no hay clave por defecto válida: un binario
# distribuido con una clave hardcodeada permitiría a cualquiera emitirse
# licencias Pro válidas. El secreto se inyecta por entorno
# (SOVNODE_LICENSE_SECRET) o se pasa explícitamente; sin él, la validación
# falla cerrada (rechaza toda licencia).
# EN: With no configured secret, there is no usable default key: a binary
# shipped with a hardcoded key would let anyone forge valid Pro licenses.
# The secret is injected via environment (SOVNODE_LICENSE_SECRET) or passed
# explicitly; without it, validation fails closed (rejects every license).
_LICENSE_SECRET_ENV_VAR = "SOVNODE_LICENSE_SECRET"

# ES: Marca de compilación opcional que el empaquetado puede sustituir por el
# secreto real; se mantiene vacía en el árbol de fuentes.
# EN: Optional build-time marker the packaging step can replace with the real
# secret; kept empty in the source tree.
...
class LicenseManager:
    ...
    def __init__(self, secret: Optional[str] = None) -> None:
        ...
        # ES: Se guarda None si no hay secreto: validate_key() falla cerrado
        # (rechaza todo) en vez de validar contra una clave pública de facto.
        # EN: Stored as None when there's no secret: validate_key() fails
        # closed (rejects everything) instead of validating against a de facto public key.
    ...
    def validate_key(self, license_key: str) -> Tuple[bool, str, Optional[LicenseInfo]]:
        ...
        if self._secret is None:
            # ES: Falla cerrado: sin secreto no hay forma de distinguir una
            # licencia legítima de una falsificada.
            # EN: Fails closed: with no secret there's no way to tell a
            # legitimate license from a forged one.
            return (
                False,
    ...
    # =================================================================
    # RELOJ ANTI-RETROCESO (trinquete monótono) / ANTI-ROLLBACK CLOCK
    # =================================================================
    #: ES: Tolerancia para desfases legítimos (zona horaria, NTP, arranque
    #: antes de sincronizar la hora). Por debajo, un reloj "hacia atrás" no
    #: se considera manipulación.
    #: EN: Tolerance for legitimate drift (timezone, NTP correction, boot
    #: before time sync). Below this, a "backwards" clock isn't treated as tampering.
    ...
    def _trusted_now(self) -> datetime:
        ...
        if previous is not None:
            ...
            if drift > self.CLOCK_ROLLBACK_TOLERANCE_SECONDS:
                # ES: Reloj retrasado más allá de lo tolerable: prevalece la marca.
                # EN: Clock set back beyond tolerance: the watermark prevails.
                effective = previous

        # ES: La marca solo avanza, nunca retrocede. / EN: The watermark only advances, never retreats.
        if previous is None or effective > previous:
            try:
                ...
                pass  # ES: no poder escribir la marca nunca debe bloquear la app / EN: failing to write the watermark must never block the app
        return effective
...
def create_license_key(
    ...
    if not resolved:
        # ES: Sin secreto no se emite nada — evita que este módulo, incluido
        # en el binario distribuido, sea una fábrica de licencias para cualquiera.
        # EN: Nothing is issued without a secret — prevents this module,
        # shipped inside the distributed binary, from being a license mill for anyone.
