...
class RobustJSONParser:
    ...
    def extract_and_repair(raw_response: str) -> Optional[Dict[str, Any]]:
        ...
        # ES: 1. Extracción directa si viene en un bloque ```json ... ```
        # EN: 1. Direct extraction if wrapped in a ```json ... ``` code block
        ...
        # ES: 2. Búsqueda por regex de cualquier objeto JSON `{...}` con clave "tool"
        # EN: 2. Regex search for any JSON object `{...}` containing a "tool" key
        ...
        # ES: 3. Último recurso: desde la primera `{` hasta la última `}`
        # EN: 3. Last resort: from the first `{` to the last `}`
    ...
    def _try_parse(json_str: str) -> Optional[Dict[str, Any]]:
        ...
        # ES: comillas simples -> dobles, y comas colgantes antes de cierres de llave/corchete
        # EN: single quotes -> double quotes, and trailing commas before closing brace/bracket
