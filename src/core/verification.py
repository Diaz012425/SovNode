...
# =====================================================================
# Generaliza verify_scores() / verify_victory_claims() (mismo pipeline:
# build_evidence_sources -> texto crudo de fuentes -> chequeo de respaldo
# léxico vía distinctive_words) a CUALQUIER afirmación cuantitativa o
# fáctica puntual extraída de una búsqueda web genérica — precio, fecha,
# cifra, versión, nombre propio — no solo marcadores/ganadores de deporte,
# que son el único dominio que verify_scores/verify_victory_claims cubren.
#
# Deliberadamente NO se pliega dentro de verify_all(): verify_all() es el
# punto de entrada ya usado por Orchestrator para el dominio deportivo con
# su propia semántica ("nunca omite ganador ni contradicciones"); mezclar
# ambos dominios ahí cambiaría el contrato para callers existentes. Este
# verificador se invoca aparte, desde el flujo de conocimiento web general
# (ver Orchestrator._persist_web_knowledge / fetch_hybrid_context).
...
def verify_web_findings(
    ...
    """
    Autoverificación de hallazgos web genéricos: coteja afirmaciones
    ...
    for match in _GENERIC_CLAIM_RE.finditer(response_text or ""):
        ...
        # NOTA IMPORTANTE: distinctive_words() excluye a propósito los
        # tokens puramente numéricos (`and not w.isdigit()`, ver
        # relevance.py) — está pensada para identificar PARTICIPANTES
        # (equipos/jugadores), no cifras. Reutilizarla tal cual dejaría
        # `numeric_words` siempre vacío y la cifra sin verificar. Por eso
        # los números se extraen aparte, directo de claim_text.
        ...
        # La cifra puntual es la parte de la afirmación que un modelo
        # alucina con más frecuencia manteniendo intacto el resto de la
        # oración (sujeto, verbo, unidad) — un solape léxico genérico
        # sobre solo las palabras no-numéricas la deja pasar igual
        # (medido: "cuesta 1999 USD" vs fuente real "cuesta 999 USD"
        # comparte sujeto/verbo/unidad y superaría cualquier umbral de
        # mitad si no se chequeara la cifra aparte). Por eso la cifra se
        # valida de forma estricta: si la afirmación trae un número, ESE
        # número tiene que aparecer literal en el texto crudo de las
        # fuentes, sin excepción.
