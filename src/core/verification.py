...
# BLINDAJE (2026-09-17, patch_orchestrator56 -- bug real, MEDIDO: "dime los
# detalles del mundial 2026" con la fecha real del torneo ya pasada -- la
# búsqueda web trajo un artículo de "pronóstico" ("...probable campeón,
# según la IA") redactado en tono afirmativo, y el marcador que mencionaba
# (2-0) aparecía literalmente en el contexto crudo recuperado, así que
# `supported_by_text` daba True aunque la fuente fuera una predicción, no
# un resultado real. Este chequeo NO reemplaza la búsqueda de texto -- solo
# la desactiva cuando TODAS las apariciones del marcador en el contexto
# caen dentro de una ventana con lenguaje claramente especulativo/
# predictivo. Si hay al menos UNA aparición sin ese lenguaje alrededor,
# sigue contando como respaldo real -- no se penaliza una fuente real solo
# porque en otra parte del contexto también haya un artículo de pronóstico.
...
def _score_support_is_purely_speculative(
    norm_claim_score: str, norm_context: str, window: int = 200,
) -> bool:
    """ES: True solo si TODAS las apariciones de `norm_claim_score` en
    `norm_context` caen dentro de una ventana con lenguaje especulativo/
    predictivo -- ver `_SPECULATIVE_OUTCOME_RE` arriba. Una sola aparición
    sin ese lenguaje alrededor alcanza para seguir contando como respaldo
    real.
    EN: True only if EVERY occurrence of `norm_claim_score` in
    `norm_context` sits inside a speculative/predictive-sounding window --
    see `_SPECULATIVE_OUTCOME_RE` above. A single occurrence without that
    language nearby is still enough to count as real support."""
    if not norm_claim_score or not norm_context:
        return False
    positions = [
        m.start() for m in re.finditer(re.escape(norm_claim_score), norm_context)
    ]
    if not positions:
        return False
    for pos in positions:
        window_text = norm_context[
            max(0, pos - window): pos + len(norm_claim_score) + window
        ]
        if not _SPECULATIVE_OUTCOME_RE.search(window_text):
            return False
    return True
...
def verify_scores(
    ...
    for claim in claimed_scores:
        ...
        if supported_by_text and _score_support_is_purely_speculative(
            norm_claim_score, norm_context,
        ):
