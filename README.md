...
├── build.py                # One-command PyInstaller packager
├── SovNode.spec             # PyInstaller spec (build.py uses this one)
...
├── src/
│   ├── core/                # Orchestration, memory, verification, background daemons
│   ├── tools/                # Tool dispatch + execution sandbox
│   └── ui/                    # PyQt6 desktop client + equation rendering
└── tests/
    └── test_regressions.py    # Regression suite (assertions, non-zero exit on failure)
```

Runtime state (`sovnode_memory.db`, `sovnode.wal`, `*.faiss` vector indices) is written next to wherever `src/core/` runs from and is intentionally excluded from git via `.gitignore` — it's your local conversation history and memory, not source code.
